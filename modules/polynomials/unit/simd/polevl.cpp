...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(polevl, NT2_REAL_TYPES )
{
 using nt2::polevl;
 ...
 typedef typename nt2::meta::call<polevl_(n_t, boost::array<rT,4> )>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(0.0, 100.0); // good value here for polevl
     }
     n_t a0 = load<n_t>(&data[0],0); 
     rn_t v  = polevl(a0, A);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
       NT2_TEST_LESSER_EQUAL( z = nt2::ulpdist(v[j], polevl(a0[j], A)), 1);
       if (z > m) m = z; 
       }
   }
