...
#else
double cephes_chbevl(), cephes_exp(), cephes_log(), cephes_sin(), cephes_lgam();
#endif
extern double PI, MAXLOG, MAXNUM;


double cephes_rgamma(x)
double x;
{
double w, y, z;
int sign;
...
if( x < -34.034 )
	...
	y = cephes_log( w * z ) - cephes_log(PI) + cephes_lgam(w);
	if( y < -MAXLOG )
		{
		cephes_mtherr( name, UNDERFLOW );
