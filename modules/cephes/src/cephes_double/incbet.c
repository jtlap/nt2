 ...
 * relation
 *
 *    1 - cephes_incbet( a, b, x )  =  cephes_incbet( b, a, 1-x ).
 *
...
#else
double cephes_gamma(), cephes_lgam(), cephes_exp(), cephes_log(), cephes_pow(), fabs();
...
if( (a+b) < MAXGAM && fabs(u) < MAXLOG )
	...
	{
	t = cephes_lgam(a+b) - cephes_lgam(a) - cephes_lgam(b) + u + cephes_log(s);
	if( t < MINLOG )
		s = 0.0;
	else
	s = cephes_exp(t);
