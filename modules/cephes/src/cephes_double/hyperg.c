 ...
 * SYNOPSIS:
 *
 * double a, b, x, y, cephes_hyperg();
 *
 * y = cephes_hyperg( a, b, x );
 *
 *
 *
 * DESCRIPTION:
...
#else
double cephes_exp(), cephes_log(), cephes_gamma(), cephes_lgam(), fabs(), cephes_hyp2f0();
...
if( fabs(temp) < 0.001 * fabs(a) )
	return( cephes_exp(x) * cephes_hyperg( temp, b, -x )  );
...
if( a < 0 )
	...
	temp = cephes_exp( t - cephes_lgam(a) );
...
double cephes_hyp2f0( a, b, x, type, err )
double a, b, x;
