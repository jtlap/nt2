 ...
 * unity.
 *
 *    log1p(x) = cephes_log(1+x)
 *    expm1(x) = cephes_exp(x) - 1
 *    cosm1(x) = cephes_cos(x) - 1
 *
 */
...
#else
double cephes_log(), cephes_polevl(), cephes_p1evl(), cephes_exp(), cephes_cos();
int isnan(), isfinite();
#endif
...
xx = x * x;
xx = -0.5*xx + xx * xx * cephes_polevl( xx, coscof, 6 );
return xx;
}
