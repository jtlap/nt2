...
#else
double cephes_pow (), cephes_floor (), cephes_lgam (), cephes_exp (), cephes_sqrt (), cephes_log (), fabs ();
     ...
     double e;
{
  ...
  if (n <= 0 || e < 0.0 || e > 1.0)
    return (-1.0);
  nn = cephes_floor ((double) n * (1.0 - e));
  p = 0.0;
  if (n < 1013)
    {
      c = 1.0;
	...
	{
	  evn = e + ((double) v) / n;
	  p += c * cephes_pow (evn, (double) (v - 1))
	    * cephes_pow (1.0 - evn, (double) (n - v));
    ...
    {
      lgamnp1 = cephes_lgam ((double) (n + 1));
      for (v = 0; v <= nn; v++)
	{
	  evn = e + ((double) v) / n;
	  omevn = 1.0 - evn;
	  if (fabs (omevn) > 0.0)
	    {
	      t = lgamnp1
		- cephes_lgam ((double) (v + 1))
		- cephes_lgam ((double) (n - v + 1))
		+ (v - 1) * cephes_log (evn)
		+ (n - v) * cephes_log (omevn);
	      if (t > -MAXLOG)
		p += cephes_exp (t);
	    }
	}
...
/* Kolmogorov's limiting distribution of two-sided test, returns
   probability that cephes_sqrt(n) * max deviation > y,
   or that max deviation > y/cephes_sqrt(n).
     ...
     double p;
{
  ...
  if (p <= 0.0 || p > 1.0)
    {
      cephes_mtherr ("smirnovi", DOMAIN);
      return 0.0;
    }
  /* Start with approximation p = cephes_exp(-2 n e^2).  */
  e = cephes_sqrt (-cephes_log (p) / (2.0 * n));
  do
    {
      /* Use approximate derivative in Newton iteration. */
      t = -2.0 * n * e;
      dpde = 2.0 * t * cephes_exp (t * e);
      if (fabs (dpde) > 0.0)
	t = (p - smirnov (n, e)) / dpde;
      else
	{
	  cephes_mtherr ("smirnovi", UNDERFLOW);
	  return 0.0;
	}
      e = e + t;
      if (e >= 1.0 || e <= 0.0)
	{
	  cephes_mtherr ("smirnovi", OVERFLOW);
	  return 0.0;
	}
    }
...
/* Functional inverse of Kolmogorov statistic for two-sided test.
   ...
   If e = smirnovi (n,p), then kolmogi(2 * p) / cephes_sqrt(n) should
   be close to e.  */
double
kolmogi (p)
     double p;
{
  ...
  if (p <= 0.0 || p > 1.0)
    {
      cephes_mtherr ("kolmogi", DOMAIN);
      return 0.0;
    }
  /* Start with approximation p = 2 cephes_exp(-2 y^2).  */
  y = cephes_sqrt (-0.5 * cephes_log (0.5 * p));
  do
    {
      /* Use approximate derivative in Newton iteration. */
      t = -2.0 * y;
      dpdy = 4.0 * t * cephes_exp (t * y);
      if (fabs (dpdy) > 0.0)
	t = (p - kolmogorov (y)) / dpdy;
      else
	{
	  cephes_mtherr ("kolmogi", UNDERFLOW);
	  return 0.0;
	}
      y = y + t;
    }
