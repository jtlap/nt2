 ...
 * DESCRIPTION:
 *
 * Computes y = cephes_exp(x*x) while suppressing error amplification
 * that would ordinarily arise from the inexactness of the
 * exponential argument x*x.
 *
 * If sign < 0, the result is inverted; i.e., y = cephes_exp(-x*x) .
 * 
 *
 * ACCURACY:
 *
     ...
     int sign;
{
  ...
  /* Represent x as an exact multiple of M plus a residual.
     M is a power of 2 chosen so that cephes_exp(m * m) does not overflow
