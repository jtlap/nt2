...
#ifdef ANSIC
float cephes_floorf(float), cephes_ldexpf(float, int), cephes_polevlf(float, float *, int);

float cephes_exp10f(float xx)
#else
float cephes_floorf(), cephes_ldexpf(), cephes_polevlf();

float cephes_exp10f(xx)
double xx;
#endif
{
float x, px, qx;
...
if( x > MAXL10 )
	{
	cephes_mtherr( "exp10f", OVERFLOW );
	return( MAXNUMF );
	}

if( x < -MAXL10 )	/* Would like to use MINLOG but can't */
	{
	cephes_mtherr( "exp10f", UNDERFLOW );
	return(0.0);
	}
