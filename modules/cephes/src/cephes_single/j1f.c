...
if( x <= 2.0f )
	{
	z = x * x;	
	p = (z-Z1) * x * cephes_polevlf( z, JP, 4 );
	return( p );
	}
...
p = w * cephes_polevlf( q, MO1, 7);
w = q*q;
xn = q * cephes_polevlf( w, PH1, 7) - THPIO4F;
...
if( x <= 2.0f )
	...
	if( x <= 0.0f )
		{
		cephes_mtherr( "y1f", DOMAIN );
		return( -MAXNUMF );
		}
	z = x * x;
	w = (z - YO1) * x * cephes_polevlf( z, YP, 4 );
	w += TWOOPI * ( cephes_j1f(x) * cephes_logf(x)  -  1.0f/x );
	return( w );
	}
