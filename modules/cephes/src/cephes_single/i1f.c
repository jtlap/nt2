...
float cephes_expf(float), cephes_sqrtf(float);
#else
...
if( z <= 8.0f )
	{
	y = 0.5f*z - 2.0f;
	z = cephes_chbevlf( y, A, 17 ) * z * cephes_expf(z);
	}
else
	{
	z = cephes_expf(z) * cephes_chbevlf( 32.0f/z - 2.0f, B, 7 ) / cephes_sqrtf(z);
	}
if( x < 0.0f )
	z = -z;
return( z );
...
#ifdef ANSIC
float cephes_i1ef( float xx )
#else
float cephes_i1ef( xx )
double xx;
#endif
{ 
float x, y, z;
...
if( z <= 8.0f )
	{
	y = 0.5f*z - 2.0f;
	z = cephes_chbevlf( y, A, 17 ) * z;
	}
else
	{
	z = cephes_chbevlf( 32.0f/z - 2.0f, B, 7 ) / cephes_sqrtf(z);
	}
if( x < 0.0f )
	z = -z;
return( z );
