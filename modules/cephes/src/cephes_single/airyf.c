 ...
 * int cephes_airyf();
 *
 * cephes_airyf( x, _&ai, _&aip, _&bi, _&bip );
 *
 *
 *
 * DESCRIPTION:
...
float cephes_sinf(float), cephes_cosf(float), cephes_expf(float), cephes_sqrtf(float);

int cephes_airyf( float xx, float *ai, float *aip, float *bi, float *bip )

#else
float cephes_polevlf(), cephes_p1evlf(), cephes_sinf(), cephes_cosf(), cephes_expf(), cephes_sqrtf();

int cephes_airyf( xx, ai, aip, bi, bip )
double xx;
float *ai, *aip, *bi, *bip;
#endif
{
