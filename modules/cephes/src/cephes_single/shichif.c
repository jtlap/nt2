...
#ifdef ANSIC
float cephes_logf(float ), cephes_expf(float), cephes_chbevlf(float, float *, int);
#else
float cephes_logf(), cephes_expf(), cephes_chbevlf();
#endif



#ifdef ANSIC
int cephes_shichif( float xx, float *si, float *ci )
#else
int cephes_shichif( xx, si, ci )
double xx;
float *si, *ci;
#endif
{
