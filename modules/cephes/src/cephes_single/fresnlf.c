...
float cephes_cosf(float), cephes_sinf(float);
#else
float cephes_polevlf(), cephes_cosf(), cephes_sinf();
#endif

#ifdef ANSIC
void cephes_fresnlf( float xxa, float *ssa, float *cca )
#else
int cephes_fresnlf( xxa, ssa, cca )
double xxa;
float *ssa, *cca;
#endif
{
