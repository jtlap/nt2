...
#ifdef ANSIC
float cephes_floorf(float), cephes_logf(float), cephes_tanf(float);
float cephes_polevlf(float, float *, int);

float cephes_psif(float xx)
#else
float cephes_floorf(), cephes_logf(), cephes_tanf(), cephes_polevlf();

float cephes_psif(xx)
double xx;
#endif
{
