 ...
 *
 * float x, y;
 * float cephes_ceilf(), cephes_floorf(), cephes_frexpf(), cephes_ldexpf();
 * int signbit(), isnan(), isfinite();
 * int expnt, n;
 *
 * y = cephes_floorf(x);
 * y = cephes_ceilf(x);
 * y = cephes_frexpf( x, &expnt );
 * y = cephes_ldexpf( x, n );
 * n = signbit(x);
 * n = isnan(x);
 * n = isfinite(x);
 ...
 *
 * cephes_ldexpf() multiplies x by 2**n.
 *
...
#ifdef ANSIC
float cephes_ceilf( float x )
#else
float cephes_ceilf(x)
double x;
#endif
{
float y;
...
#ifdef ANSIC
float cephes_floorf( float x )
#else
float cephes_floorf(x)
double x;
#endif
{
unsigned short *p;
...
#ifdef ANSIC
float cephes_frexpf( float x, int *pw2 )
#else
float cephes_frexpf( x, pw2 )
double x;
int *pw2;
#endif
{
...
#ifdef ANSIC
float cephes_ldexpf( float x, int pw2 )
#else
float cephes_ldexpf( x, pw2 )
double x;
int pw2;
#endif
{
