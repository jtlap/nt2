...
float cephes_expf(float), cephes_logf(float), cephes_sinf(float), cephes_lgamf(float);

float cephes_rgammaf(float xx)
#else
float cephes_chbevlf(), cephes_expf(), cephes_logf(), cephes_sinf(), cephes_lgamf();

float cephes_rgammaf(xx)
double xx;
#endif
{
float x, w, y, z;
...
if( x < -34.034f )
	...
	y = cephes_logf( w * z / PIF ) + cephes_lgamf(w);
	if( y < -MAXLOGF )
		{
		cephes_mtherr( name, UNDERFLOW );
