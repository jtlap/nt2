 ...
 * SYNOPSIS:
 *
 * long double p, t, cephes_stdtril();
 * int k;
 *
 * t = cephes_stdtril( k, p );
 *
 *
 * DESCRIPTION:
 *
 * Given probability p, finds the argument t such that cephes_stdtrl(k,t)
 * is equal to p.
 * 
 * ACCURACY:
 *
...
#else
long double cephes_sqrtl(), cephes_atanl(), cephes_incbetl(), cephes_incbil(), cephes_fabsl();
#endif

long double cephes_stdtrl( k, t )
int k;
long double t;
{
...
if( t < -1.6L )
	...
	rk = k;
	z = rk / (rk + t * t);
	p = 0.5L * cephes_incbetl( 0.5L*rk, 0.5L, z );
	return( p );
	}
...
long double cephes_stdtril( k, p )
int k;
long double p;
{
long double t, rk, z;
...
if( p > 0.25L && p < 0.75L )
	...
	if( p == 0.5L )
		return( 0.0L );
	z = 1.0L - 2.0L * p;
	z = cephes_incbil( 0.5L, 0.5L*rk, cephes_fabsl(z) );
	t = cephes_sqrtl( rk*z/(1.0L-z) );
	if( p < 0.5L )
		t = -t;
	return( t );
	}
...
z = cephes_incbil( 0.5L*rk, 0.5L, 2.0L*p );

if( MAXNUML * z < rk )
	return(rflg* MAXNUML);
t = cephes_sqrtl( rk/z - rk );
return( rflg * t );
}
