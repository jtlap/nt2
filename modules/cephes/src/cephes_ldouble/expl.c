...
#else
long double cephes_polevll(), cephes_floorl(), cephes_ldexpl(), cephes_isnanl();
#endif
#ifdef INFINITIES
extern long double INFINITYL;
#endif

long double cephes_expl(x)
long double x;
{
long double px, xx;
int n;
 ...
 *   = e**( g + n loge(2) )
 */
px = cephes_floorl( LOG2EL * x + 0.5L ); /* floor() truncates toward -infinity. */
n = px;
x -= px * C1;
x -= px * C2;
