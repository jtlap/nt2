...
NT2_TEST_CASE_TPL ( unary_minus_real,  BOOST_SIMD_REAL_TYPES)
{
  ...
  typedef typename boost::dispatch::meta::call<unary_minus_(cT,cT)>::type r_t;
...
#endif
  NT2_TEST_EQUAL(nt2::unary_minus(cT(nt2::One<T>())), cT(nt2::One<T>()));
  NT2_TEST_EQUAL(nt2::unary_minus(cT(nt2::Zero<T>()), cT(nt2::Zero<T>())), cT(nt2::Zero<T>()));
