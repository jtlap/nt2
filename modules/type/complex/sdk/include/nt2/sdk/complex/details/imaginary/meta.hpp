...
{
  template<class T>
  struct model_of< nt2::imaginary<T> >
  {
    struct type
    {
      template<class U> struct apply
      {
        typedef  nt2::imaginary<U> type;
      };
    };
  };
...
{
  template<class T>
  struct has_trivial_copy< nt2::imaginary<T> >
       : boost::mpl::true_
  {
  };

  template<class T>
  struct has_trivial_default_constructor< nt2::imaginary<T> >
       : boost::mpl::true_
  {
  };

  template<class T>
  struct has_trivial_destructor< nt2::imaginary<T> >
       : boost::mpl::true_
  {
  };
}
