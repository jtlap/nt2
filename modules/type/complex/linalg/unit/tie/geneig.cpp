...
NT2_TEST_CASE_TPL(geneig_sca, NT2_REAL_TYPES)
{
  ...
  ct_t w =   geneig(cT(2), cT(2));
  NT2_TEST_ULP_EQUAL(w, cT(1), 0);
  w =   geneig(cT(2),cT(2), nt2::matrix_);
  NT2_TEST_ULP_EQUAL(w, cT(1), 0);
  w =   geneig(cT(2),cT(2), nt2::vector_);
  NT2_TEST_ULP_EQUAL(w, cT(1), 0);
}
...
NT2_TEST_CASE_TPL(geneig_1o_ni, NT2_REAL_TYPES)
{
  ...
  typedef nt2::table<cT> ct_t;
  ct_t a =  nt2::cons(nt2::of_size(2, 2), nt2::Sqrt_2o_2<T>(), T(0), T(0), T(1));
  ct_t b =  nt2::I<cT>()*nt2::cons(nt2::of_size(2, 2), T(0), -nt2::Sqrt_2o_2<T>(), T(1), T(0));
  ct_t w, w1, wd;
