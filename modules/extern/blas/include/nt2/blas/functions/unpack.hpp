...
//==============================================================================
#ifndef NT2_BLAS_FUNCTIONS_BLAS_UNPACK_HPP_INCLUDED
#define NT2_BLAS_FUNCTIONS_BLAS_UNPACK_HPP_INCLUDED

/*!
  @file
  @brief Defines and implements unpack function
**/
...
namespace nt2 { namespace blas { namespace tag
  {
    /*!
     * \brief Define the tag unpack_ of functor unpack
     *        in namespace nt2 { namespace blas::tag for toolbox algebra
     **/
    struct unpack_ : ext::unspecified_<unpack_>
    {
      typedef ext::unspecified_<unpack_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_unpack_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
}
  namespace ext
  {
    template<class Site>
    BOOST_FORCEINLINE generic_dispatcher<tag::unpack_, Site> dispatching_unpack_(adl_helper, boost::dispatch::meta::unknown_<Site>, ...)
    {
      return generic_dispatcher<tag::unpack_, Site>();
    }
    template<class... Args>
    struct impl_unpack_;
  }

  NT2_FUNCTION_IMPLEMENTATION(tag::unpack_, unpack, 2)
  NT2_FUNCTION_IMPLEMENTATION(tag::unpack_, unpack, 3)
  /*!
...
namespace nt2 { namespace blas { namespace ext
{
  ...
  struct  size_of<tag::unpack_, Domain, N, Expr>
  {
  ...
  template <class Domain, class Expr, int N>
  struct value_type < tag::unpack_, Domain,N,Expr>
  {
