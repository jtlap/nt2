...
//==============================================================================
#ifndef NT2_BLAS_FUNCTIONS_BLAS_SYMM_HPP_INCLUDED
#define NT2_BLAS_FUNCTIONS_BLAS_SYMM_HPP_INCLUDED
...
namespace nt2 { namespace blas
{
  namespace tag
  {
    ...
    struct symm_ : ext::abstract_<symm_>
    {
      /// INTERNAL ONLY
      typedef ext::abstract_<symm_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_symm_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
    template<class Site>
    BOOST_FORCEINLINE generic_dispatcher<tag::symm_, Site> dispatching_symm_(adl_helper, boost::dispatch::meta::unknown_<Site>, ...)
    {
      return generic_dispatcher<tag::symm_, Site>();
    }
    template<class... Args>
    struct impl_symm_;
  }
  ...
  /*!
    @brief symm - perform one of the following matrix-matrix operation

     c = alpha*a*b + beta*c,
     c = alpha*b*a + beta*c

     where alpha and beta are scalars, A is an symmetric matrix and  B and C
     are m by n matrices.
  ...
  **/
  NT2_FUNCTION_IMPLEMENTATION_TPL (tag::symm_, symm
