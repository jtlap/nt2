...
#ifdef ANSIC
float cephes_expf(float), cephes_logf(float), cephes_sqrtf(float);

float cephes_knf( int nnn, float xx )
#else
float cephes_expf(), cephes_logf(), cephes_sqrtf();

float cephes_knf( nnn, xx )
int nnn;
double xx;
#endif
{
...
if( x <= 0.0f )
	{
	if( x < 0.0f )
		cephes_mtherr( "knf", DOMAIN );
	else
		cephes_mtherr( "knf", SING );
	return( MAXNUMF );
	}
...
adone:
ans = cephes_expf(-x) * cephes_sqrtf( PIF/(2.0f*x) ) * s;
return(ans);
}
