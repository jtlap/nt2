 ...
 * cephes_polprtf( a, na, D );		Print the coefficients of a to D digits.
 * cephes_polclrf( a, na );		Set a identically equal to zero, up to a[na].
 * cephes_polmovf( a, na, b );		Set b = a.
 * cephes_poladdf( a, na, b, nb, c );	c = b + a, nc = max(na,nb)
 * cephes_polsubf( a, na, b, nb, c );	c = b - a, nc = max(na,nb)
 * cephes_polmulf( a, na, b, nb, c );	c = b * a, nc = na+nb
 *
 *
 * Division:
 *
 * i = cephes_poldivf( a, na, b, nb, c );	c = b / a, nc = MAXPOL
 *
 ...
 *
 * Notes:
 * cephes_poldivf() is an integer routine; cephes_polevaf() is float.
 ...
 */
#ifdef ANSIC
void cephes_polinif( int maxdeg )
#else
int cephes_polinif( maxdeg )
int maxdeg;
#endif
{
...
#ifdef ANSIC
void cephes_polprtf( float *a, int na, int d )
#else
int cephes_polprtf( a, na, d )
float a[];
int na, d;
#endif
{
 ...
 */
#ifdef ANSIC
void cephes_polclrf( register float *a, int n )
#else
int cephes_polclrf( a, n )
register float *a;
int n;
#endif
{
 ...
 */
#ifdef ANSIC
void cephes_polmovf( register float *a, int na, register float *b )
#else
int cephes_polmovf( a, na, b )
register float *a, *b;
int na;
#endif
{
 ...
 */
#ifdef ANSIC
void cephes_polmulf( float a[], int na, float b[], int nb, float c[] )
#else
int cephes_polmulf( a, na, b, nb, c )
float a[], b[], c[];
int na, nb;
#endif
{
 ...
 */
#ifdef ANSIC
void cephes_poladdf( float a[], int na, float b[], int nb, float c[] )
#else
int cephes_poladdf( a, na, b, nb, c )
float a[], b[], c[];
int na, nb;
#endif
{
 ...
 */
#ifdef ANSIC
void cephes_polsubf( float a[], int na, float b[], int nb, float c[] )
#else
int cephes_polsubf( a, na, b, nb, c )
float a[], b[], c[];
int na, nb;
#endif
{
 ...
 */
#ifdef ANSIC
int cephes_poldivf( float a[], int na, float b[], int nb, float c[] )
#else
int cephes_poldivf( a, na, b, nb, c )
float a[], b[], c[];
int na, nb;
#endif
{
...
cephes_polclrf( ta, MAXPOLF );
cephes_polmovf( a, na, ta );

tb = (float * )malloc( psize );
cephes_polclrf( tb, MAXPOLF );
cephes_polmovf( b, nb, tb );

tq = (float * )malloc( psize );
cephes_polclrf( tq, MAXPOLF );
...
#ifdef ANSIC
void cephes_polsbt( float a[], int na, float b[], int nb, float c[] )
#else
int cephes_polsbt( a, na, b, nb, c )
float a[], b[], c[];
int na, nb;
#endif
{
...
for( i=1; i<=nb; i++ )
	{
/* Form ith power of a. */
	cephes_polmulf( a, na, pt2, n2, pt2 );
	n2 += na;
	x = b[i];
 ...
 */
#ifdef ANSIC
float cephes_polevaf( float *a, int na, float xx )
#else
float cephes_polevaf( a, na, xx )
float a[];
int na;
double xx;
#endif
