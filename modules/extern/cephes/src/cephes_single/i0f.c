...
#ifdef ANSIC
float cephes_chbevlf(float, float *, int), cephes_expf(float), cephes_sqrtf(float);

float cephes_i0f( float x )
#else
float cephes_chbevlf(), cephes_expf(), cephes_sqrtf();
...
if( x <= 8.0f )
	{
	y = 0.5f*x - 2.0f;
	return( cephes_expf(x) * cephes_chbevlf( y, A, 18 ) );
	}

return(  cephes_expf(x) * cephes_chbevlf( 32.0f/x - 2.0f, B, 7 ) / cephes_sqrtf(x) );
}
...
if( x <= 8.0f )
	{
	y = 0.5f*x - 2.0f;
	return( cephes_chbevlf( y, A, 18 ) );
	}

return(  cephes_chbevlf( 32.0f/x - 2.0f, B, 7 ) / cephes_sqrtf(x) );
}
