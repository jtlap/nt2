...
#ifdef ANSIC
float cephes_logf(float), cephes_sinf(float), cephes_cosf(float);
float cephes_polevlf(float, float *, int);
float cephes_p1evlf(float, float *, int);
#else
float cephes_logf(), cephes_sinf(), cephes_cosf(), cephes_polevlf(), cephes_p1evlf();
#endif


#ifdef ANSIC
int cephes_sicif( float xx, float *si, float *ci )
#else
int cephes_sicif( xx, si, ci )
double xx;
float *si, *ci;
#endif
{
...
z = x * x;
s = x * cephes_polevlf( z, SN, 5 ) / cephes_polevlf( z, SD, 5 );
c = z * cephes_polevlf( z, CN, 5 ) / cephes_polevlf( z, CD, 5 );

if( sign )
	s = -s;
*si = s;
*ci = EUL + cephes_logf(x) + c;	/* real part if x < 0 */
return(0);
...
if( x < 8.0f )
	...
	{
	f = cephes_polevlf( z, FN8, 8 ) / (x * cephes_p1evlf( z, FD8, 8 ));
	g = z * cephes_polevlf( z, GN8, 8 ) / cephes_p1evlf( z, GD8, 9 );
	}
*si = PIO2F - f * c - g * s;
if( sign )
	*si = -( *si );
