 ...
 * SYNOPSIS:
 *
 * float a, b, x, y, cephes_hypergf();
 *
 * y = cephes_hypergf( a, b, x );
 *
 *
 *
 * DESCRIPTION:
...
float cephes_logf(float), cephes_gammaf(float), cephes_lgamf(float);
#else
float cephes_expf(), cephes_hyp2f0f();
float cephes_logf(), cephes_gammaf(), cephes_lgamf();
static float hy1f1pf(), hy1f1af();
#endif

#ifdef ANSIC
float cephes_hypergf( float aa, float bb, float xx )
#else
float cephes_hypergf( aa, bb, xx)
double aa, bb, xx;
#endif
{
...
if( a < 0 )
	...
	temp = cephes_expf( t - cephes_lgamf(a) );
...
#ifdef ANSIC
float cephes_hyp2f0f(float aa, float bb, float xx, int type, float *err)
#else
float cephes_hyp2f0f( aa, bb, xx, type, err )
double aa, bb, xx;
