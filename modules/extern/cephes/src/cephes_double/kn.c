...
For large x,
                                         2        2     2
                                      u-1     (u-1 )(u-3 )
K (z)  =  cephes_sqrt(pi/2z) cephes_exp(-z) { 1 + ------- + ------------ + ...}
 v                                        1            2
                                    1! (8z)     2! (8z)
...
#else
double fabs(), cephes_exp(), cephes_log(), cephes_sqrt();
#endif
extern double MACHEP, MAXNUM, MAXLOG, PI;

double cephes_kn( nn, x )
int nn;
double x;
{
...
if( x <= 0.0 )
	{
	if( x < 0.0 )
		cephes_mtherr( "kn", DOMAIN );
	else
		cephes_mtherr( "kn", SING );
	return( MAXNUM );
	}
...
if( n > 0 )
	{
	/* compute factorial of n and cephes_psi(n) */
	pn = -EUL;
	k = 1.0;
	for( i=1; i<n; i++ )
		{
