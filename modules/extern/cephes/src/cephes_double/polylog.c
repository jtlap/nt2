...
/* polylog(4, 1-x) = cephes_zeta(4) - x cephes_zeta(3) + x^2 A4(x)/B4(x)
   0 <= x <= 0.125
...
extern double cephes_pow(), cephes_powi(), cephes_log();
 ...
 *                                                   [n/2]   n-2r
 *                n                  1     n           -  cephes_log    (z)
 *  Li (-z) + (-1)  Li (-1/z)  =  - --- cephes_log (z)  +  2  >  ----------- Li  (-1)
 *    n               n              n!                -   (n - 2r)!    2r
 *                                                    r=1
 */
  if (x < -1.0 && n > 1)
      ...
      for (r = 1; r <= n / 2; r++)
	{
	  ...
	  q = (double) j;
	  q = cephes_pow (w, q) * p / cephes_fac (j);
	  s = s + q;
	}
      s = 2.0 * s;
      q = polylog (n, 1.0 / x);
	...
	q = -q;
      s = s - q;
      s = s - cephes_pow (w, (double) n) / cephes_fac (n);
      return s;
    }
  ...
  /*  The power series converges slowly when x is near 1.  For n = 3, this
      ...
      Li (-x/(1-x)) + Li (1-x) + Li (x)
        3               3          3
                     2                               2                 3
       = Li (1) + (pi /6) cephes_log(1-x) - (1/2) cephes_log(x) cephes_log (1-x) + (1/6) cephes_log (1-x)
           3
  */
...
/*  This expansion in powers of cephes_log(x) is especially useful when
    x is near 1.
    ...
    polylog(n,x)  =    >   -----------------
      ...
      z(j) = Riemann zeta function (j), j != 1
                              ...
                              n-1
                               -
      z(1) =  -cephes_log(-cephes_log(x)) +  >  1/k
                               -
                              k=1
  */
