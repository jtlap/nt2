...
/* Chebyshev coefficients for K0(x) + cephes_log(x/2) I0(x)
 * in the interval [0,2].  The odd order coefficients are all
 * zero; only the even order coefficients are listed.
 * 
 * lim(x->0){ K0(x) + cephes_log(x/2) I0(x) } = -EUL.
 */
...
/* Chebyshev coefficients for cephes_exp(x) cephes_sqrt(x) K0(x)
 * in the inverted interval [2,infinity].
 * 
 * lim(x->inf){ cephes_exp(x) cephes_sqrt(x) K0(x) } = cephes_sqrt(pi/2).
 */
...
#else
double cephes_chbevl(), cephes_exp(), cephes_i0(), cephes_log(), cephes_sqrt();
#endif
extern double PI;
