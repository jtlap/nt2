 ...
 *
 *    cephes_asin(x) = pi/2 - 2 cephes_asin( cephes_sqrt( (1-x)/2 ) ).
 *
 *
 * ACCURACY:
 *
 ...
 * is x.
 *
 * Analytically, cephes_acos(x) = pi/2 - cephes_asin(x).  However if |x| is
 * near 1, there is cancellation error in subtracting cephes_asin(x)
 * from pi/2.  Hence if x < -0.5,
 *
 *    cephes_acos(x) =	 pi - 2.0 * cephes_asin( cephes_sqrt((1+x)/2) );
 *
 * or if x > +0.5,
 *
 *    cephes_acos(x) =	 2.0 * cephes_asin(  cephes_sqrt((1-x)/2) ).
 *
 *
 * ACCURACY:
 *
...
#else
double cephes_sqrt(), cephes_polevl(), cephes_p1evl();
double cephes_asin();
#endif
