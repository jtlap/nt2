...
#else
double cephes_gamma(), cephes_pow(), cephes_sqrt(), cephes_yn(), cephes_yv(), cephes_jv(), fabs(), cephes_floor();
...
double cephes_onef2( a, b, c, x, err )
double a, b, c, x;
double *err;
{
double n, a0, sum, t;
...
double cephes_threef0( a, b, c, x, err )
double a, b, c, x;
double *err;
{
...
t = PI * v;
y = (cephes_cos(t) * cephes_jv( v, x ) - cephes_jv( -v, x ))/cephes_sin(t);
return( y );
}
