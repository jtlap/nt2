 ...
 *   cephes_y0(x)  = R(x)  +   2 * cephes_log(x) * cephes_j0(x) / PI.
 * Thus a call to cephes_j0() is required.
 *
 * In the second interval, the Hankel asymptotic expansion
 * is employed with two rational functions of degree 6/6
 * and 7/7.
 ...
 * ACCURACY:
 *
 *  Absolute error, when cephes_y0(x) < 1; else relative error:
 *
 * arithmetic   domain     # trials      peak         rms
...
#else
double cephes_polevl(), cephes_p1evl(), cephes_log(), cephes_sin(), cephes_cos(), cephes_sqrt();
double cephes_j0();
#endif
...
if( x <= 5.0 )
	...
	p = (z - DR1) * (z - DR2);
	p = p * cephes_polevl( z, (double*)RP, 3)/cephes_p1evl( z, (double*)RQ, 8 );
	return( p );
	}

w = 5.0/x;
q = 25.0/(x*x);
p = cephes_polevl( q, (double*)PP, 6)/cephes_polevl( q, (double*)PQ, 6 );
q = cephes_polevl( q, (double*)QP, 7)/cephes_p1evl( q, (double*)QQ, 7 );
xn = x - PIO4;
 ...
 * The function computed is  cephes_y0(x)  -  2 * cephes_log(x) * cephes_j0(x) / PI,
 * whose value at x = 0 is  2 * ( cephes_log(0.5) + EUL ) / PI
...
if( x <= 5.0 )
	...
	if( x <= 0.0 )
		{
		cephes_mtherr( "y0", DOMAIN );
		return( -MAXNUM );
		}
	z = x * x;
	w = cephes_polevl( z, (double*)YP, 7) / cephes_p1evl( z, (double*)YQ, 7 );
