 ...
 * employed.  Otherwise,
 *        cephes_atanh(x) = 0.5 * cephes_log( (1+x)/(1-x) ).
 *
 *
 *
 * ACCURACY:
...
extern double cephes_polevl ( double x, double *P, int N );
extern double cephes_p1evl ( double x, double *P, int N );
#else
double fabs(), cephes_log(), cephes_polevl(), cephes_p1evl();
#endif
