 ...
 *
 *      cephes_k1e(x) = cephes_exp(x) * cephes_k1(x).
 *
 *
 *
 * ACCURACY:
...
/* Chebyshev coefficients for x(K1(x) - cephes_log(x/2) I1(x))
 * in the interval [0,2].
 * 
 * lim(x->0){ x(K1(x) - cephes_log(x/2) I1(x)) } = 1.
 */
...
/* Chebyshev coefficients for cephes_exp(x) cephes_sqrt(x) K1(x)
 * in the interval [2,infinity].
 *
 * lim(x->inf){ cephes_exp(x) cephes_sqrt(x) K1(x) } = cephes_sqrt(pi/2).
 */
...
#else
double cephes_chbevl(), cephes_exp(), cephes_i1(), cephes_log(), cephes_sqrt();
#endif
extern double PI;
...
if( x <= 2.0 )
	{
	y = x * x - 2.0;
	y =  cephes_log(z) * cephes_i1(x)  +  cephes_chbevl( y, (double*)A, 11 ) / x;
	return( y );
	}

return(  cephes_exp(-x) * cephes_chbevl( 8.0/x - 2.0, (double*)B, 25 ) / cephes_sqrt(x) );
}
...
if( x <= 2.0 )
	{
	y = x * x - 2.0;
	y =  cephes_log( 0.5 * x ) * cephes_i1(x)  +  cephes_chbevl( y, (double*)A, 11 ) / x;
	return( y * cephes_exp(x) );
	}

return(  cephes_chbevl( 8.0/x - 2.0, (double*)B, 25 ) / cephes_sqrt(x) );
}
