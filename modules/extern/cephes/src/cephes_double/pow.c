 ...
 * 1/26 < x < 26, with cephes_log(x) uniformly distributed.
...
#else
double cephes_floor(), fabs(), cephes_frexp(), cephes_ldexp();
double cephes_polevl(), cephes_p1evl(), cephes_powi();
...
/* Find (x - A[i])/A[i]
 * in order to compute cephes_log(x/A[i]):
 *
 * cephes_log(x) = cephes_log( a x/a ) = cephes_log(a) + cephes_log(x/a)
 *
 * cephes_log(x/a) = cephes_log(1+v),  v = x/a - 1 = (x-a)/a
 */
x -= douba(i);
x -= doubb(i/2);
x /= douba(i);
...
/* rational approximation for cephes_log(1+v):
 *
...
/* Now the product y * cephes_log2(x)  =  Hb + e/16.0.
 *
