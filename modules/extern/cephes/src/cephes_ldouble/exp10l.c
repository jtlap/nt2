...
#else
long double cephes_floorl(), cephes_ldexpl(), cephes_polevll(), cephes_p1evll(), cephes_isnanl();
#endif
#ifdef INFINITIES
extern long double INFINITYL;
#endif

long double cephes_exp10l(x)
long double x;
{
long double px, xx;
short n;
...
if( x < -MAXL10 )	/* Would like to use MINLOG but can't */
	{
#ifndef INFINITIES
	cephes_mtherr( "exp10l", UNDERFLOW );
#endif
	return(0.0L);
	}
 ...
 *   = 10**( g + n log10(2) )
 */
px = cephes_floorl( LOG210 * x + 0.5L );
n = px;
x -= px * LG102A;
x -= px * LG102B;
 ...
 */
xx = x * x;
px = x * cephes_polevll( xx, (long double*)P, 3 );
x =  px/( cephes_p1evll( xx, (long double*)Q, 4 ) - px );
