...
#else
long double cephes_logl(), cephes_sqrtl(), cephes_polevll(), cephes_p1evll(), cephes_isnanl(), cephes_isfinitel();
#endif

long double cephes_asinhl(x)
long double x;
{
long double a, z;
int sign;
...
if( x > 1.0e10L )
	{
	return( sign * (cephes_logl(x) + LOGE2L) );
	}
...
a = cephes_sqrtl( z + 1.0L );
return( sign * cephes_logl(x + a) );
}
