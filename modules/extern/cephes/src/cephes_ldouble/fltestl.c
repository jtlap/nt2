...
long double cephes_frexpl(), cephes_ldexpl(), cephes_floorl();
...
printf( "Testing cephes_frexpl() and cephes_ldexpl().\n" );
errth = 0.0L;
errfr = 0;
errld = 0;
underexp = 0;
...
for( i=0; i<16383; i++ )
	...
	if( (x != 0.0L) && ((err > errth) || (e != e0)) )
		{
		printf( "Test %d: ", j+1 );
		printf( " cephes_frexpl( %.20Le) =?= %.20Le * 2**%d;", x, y, e );
		printf( " should be %.20Le * 2**%d\n", y0, e0 );
		errfr += 1;
		}
	y = cephes_ldexpl( x, 1-e0 );
	err = y - 1.0L;
	if( err < 0.0L )
		err = -err;
...
y = cephes_floorl(-1.0L);
if( y != -1.0L )
	{
	flierr( -1.0L, y, 59 );
	errfl += 1;
	}
y = cephes_floorl(-0.1L);
if( y != -1.0l )
	{
	flierr( -0.1L, y, 60 );
	errfl += 1;
...
if( errfl == 0 )
	printf( "No errors found in cephes_floorl().\n" );
exit(0);
return 0;
}
