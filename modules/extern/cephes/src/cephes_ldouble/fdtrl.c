 ...
 *
 * int df1, df2;
 * long double x, y, cephes_fdtrl();
 *
 * y = cephes_fdtrl( df1, df2, x );
 *
 *
 *
 * DESCRIPTION:
 ...
 * formula
 *
 *	P(x) = cephes_incbetl( df1/2, df2/2, (df1*x/(df2 + df1*x) ).
 *
 *
 ...
 *
 * int df1, df2;
 * long double x, y, cephes_fdtrcl();
 *
 * y = cephes_fdtrcl( df1, df2, x );
 *
 *
 *
 * DESCRIPTION:
 ...
 *
 * int df1, df2;
 * long double x, p, cephes_fdtril();
 *
 * x = cephes_fdtril( df1, df2, p );
 *
 * DESCRIPTION:
 *
...
#else
long double cephes_incbetl(), cephes_incbil();
#endif

long double cephes_fdtrcl( ia, ib, x )
int ia, ib;
long double x;
{
long double a, b, w;
...
if( (ia < 1) || (ib < 1) || (x < 0.0L) )
	{
	cephes_mtherr( "fdtrcl", DOMAIN );
	return( 0.0L );
	}
a = ia;
b = ib;
w = b / (b + a * x);
return( cephes_incbetl( 0.5L*b, 0.5L*a, w ) );
}
...
w = a * x;
w = w / (b + w);
return( cephes_incbetl(0.5L*a, 0.5L*b, w) );
}


long double cephes_fdtril( ia, ib, y )
int ia, ib;
long double y;
{
long double a, b, w, x;
...
if( (ia < 1) || (ib < 1) || (y <= 0.0L) || (y > 1.0L) )
	{
	cephes_mtherr( "fdtril", DOMAIN );
	return( 0.0L );
	}
a = ia;
b = ib;
/* Compute probability for x = 0.5.  */
w = cephes_incbetl( 0.5L*b, 0.5L*a, 0.5L );
/* If that is greater than y, then the solution w < .5.
   Otherwise, solve at 1-y to remove cancellation in (b - b*w).  */
if( w > y || y < 0.001L)
	{
	w = cephes_incbil( 0.5L*b, 0.5L*a, y );
	x = (b - b*w)/(a*w);
	}
else
	{
	w = cephes_incbil( 0.5L*a, 0.5L*b, 1.0L - y );
	x = b*w/(a*(1.0L-w));
	}
return(x);
}
