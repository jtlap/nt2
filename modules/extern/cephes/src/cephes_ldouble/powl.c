...
#else
long double cephes_floorl(), cephes_fabsl(), cephes_frexpl(), cephes_ldexpl();
long double cephes_polevll(), cephes_p1evll(), cephes_powil();
...
#ifdef INFINITIES
if( !cephes_isfinitel(y) && (x == -1.0L || x == 1.0L) )
	{
	cephes_mtherr( "powl", DOMAIN );
#ifdef NANS
	return( NANL );
#else
	return( INFINITYL );
...
if( iyflg )
	{
	ya = cephes_fabsl(y);
	ya = cephes_floorl(0.5L * ya);
	yb = 0.5L * cephes_fabsl(w);
	if( ya != yb )
		yoddint = 1;
	}
...
if( iyflg )
	...
	i = w;
	w = cephes_floorl(x);
	if( (w == x) && (cephes_fabsl(y) < 32768.0) )
		{
		w = cephes_powil( x, (int) y );
		return( w );
		}
	}
 ...
 */
z = x*x;
w = x * ( z * cephes_polevll( x, (long double*)P, 3 ) / cephes_p1evll( x, (long double*)Q, 3 ) );
w = w - cephes_ldexpl( z, -1 );   /*  w - 0.5 * z  */
...
w = -i;
w = cephes_ldexpl( w, -LNXT );	/* divide by NXT */
w += e;
...
H = Fb + Gb;
Ha = reducl(H);
w = cephes_ldexpl( Ga+Ha, LNXT );

/* Test the power of 2 for overflow */
if( w > MEXP )
	{
/*	printf( "w = %.4Le ", w ); */
	cephes_mtherr( fname, OVERFLOW );
	return( MAXNUML );
	}
 ...
 * where -0.0625 <= Hb <= 0.
 */
z = Hb * cephes_polevll( Hb, (long double*)R, 6 );  /*    z  =  2**Hb - 1    */
...
z = w * z;      /*    2**-e * ( 1 + (2**Hb-1) )    */
z = z + w;
z = cephes_ldexpl( z, i );  /* multiply by integer power of 2 */
...
t = cephes_ldexpl( x, LNXT );
t = cephes_floorl( t );
t = cephes_ldexpl( t, -LNXT );
return(t);
}
