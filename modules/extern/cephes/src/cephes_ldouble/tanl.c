...
#else
long double cephes_polevll(), cephes_p1evll(), cephes_floorl(), cephes_ldexpl(), cephes_isnanl(), cephes_isfinitel();
...
long double cephes_cotl(x)
long double x;
{

if( x == 0.0L )
	{
	cephes_mtherr( "cotl", SING );
#ifdef INFINITIES
	return( INFINITYL );
#else
	return( MAXNUML );
...
if( x > lossth )
	{
	if( cotflg )
		cephes_mtherr( "cotl", TLOSS );
	else
		cephes_mtherr( "tanl", TLOSS );
	return(0.0L);
	}

/* compute x mod PIO4 */
y = cephes_floorl( x/PIO4L );

/* strip high bits of integer part */
z = cephes_ldexpl( y, -4 );
z = cephes_floorl(z);		/* integer part of y/16 */
z = y - cephes_ldexpl( z, 4 );  /* y - 16 * (y/16) */
