...
long double cephes_cabsl ( cmplxl *x );
void cephes_csqrtl ( cmplxl *x, cmplxl *y );
void cephes_caddl ( cmplxl *x, cmplxl *y, cmplxl *z );
...
long double cephes_cabsl(), cephes_fabsl(), cephes_sqrtl();
lnog double cephes_logl(), cephes_expl(), cephes_atan2l(), cephes_coshl(), cephes_sinhl();
long double cephes_asinl(), cephes_sinl(), cephes_cosl();
void cephes_caddl(), cephes_csqrtl(), clogl(), casinl();
#endif
...
p = cephes_logl(rr);
#if ANSIC
rr = cephes_atan2l( z->i, z->r );
#else
rr = cephes_atan2l( z->r, z->i );
if( rr > PIL )
	rr -= PIL + PIL;
#endif
w->i = rr;
...
r = cephes_expl( z->r );
w->r = r * cephes_cosl( z->i );
w->i = r * cephes_sinl( z->i );
}
/*							csinl()
 *
...
cchshl( z->i, &ch, &sh );
w->r = cephes_sinl( z->r ) * ch;
w->i = cephes_cosl( z->r ) * sh;
}
...
cchshl( z->i, &ch, &sh );
w->r = cephes_cosl( z->r ) * ch;
w->i = -cephes_sinl( z->r ) * sh;
}
/*							ctanl()
 *
...
d = cephes_cosl( 2.0L * z->r ) + cephes_coshl( 2.0L * z->i );

if( cephes_fabsl(d) < 0.25L )
	d = ctansl(z);
...
w->r = cephes_sinl( 2.0L * z->r ) / d;
w->i = cephes_sinhl( 2.0L * z->i ) / d;
}
/*							ccotl()
 *
...
d = cephes_coshl(2.0L * z->i) - cephes_cosl(2.0L * z->r);
...
x = cephes_fabsl( 2.0L * z->r );
y = cephes_fabsl( 2.0L * z->i );
...
while( cephes_fabsl(t/d) > MACHEPL );
return(d);
}
/*							casinl()
 *
...
if( y == 0.0L )
	{
	if( cephes_fabsl(x) > 1.0L )
		{
		w->r = PIO2L;
		w->i = 0.0L;
		cephes_mtherr( "casinl", DOMAIN );
		}
	else
		{
		w->r = cephes_asinl(x);
...
#if ANSIC
t = cephes_atan2l( 2.0L * x, a ) * 0.5L;
#else
t = cephes_atan2l( a, 2.0 * x ) * 0.5L;
#endif
w->r = redupil( t );
...
t = y + 1.0L;
a = (x2 + (t * t))/a;
w->i = cephes_logl(a)/4.0;
return;

ovrf:
cephes_mtherr( "catanl", OVERFLOW );
w->r = MAXNUML;
w->i = MAXNUML;
}
