...
NT2_REGISTER_DISPATCH ( tag::is_less_, tag::cpu_, (X)(A0)(A1)
                      , ((simd_<fundamental_<A0>, X>))((simd_<fundamental_<A1>, X>))
                      )
namespace nt2 { namespace ext
{
  template<class X,class Dummy>
  struct call<tag::is_less_(tag::simd_<tag::fundamental_,X>, tag::simd_<tag::fundamental_,X>), tag::cpu_, Dummy>
        : callable
  {
    ...
    NT2_FUNCTOR_CALL(2)
    {
      A0 that = map ( functor<tag::genmask_>()
                    , map(functor<tag::is_less_>(), a0, a1)
                    );
      return that;
    }
  };
