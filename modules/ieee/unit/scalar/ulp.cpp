...
NT2_TEST_CASE_TPL ( ulp_real__1_0,  NT2_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(ulp(nt2::Inf<T>()), nt2::Inf<r_t>());
  NT2_TEST_EQUAL(ulp(nt2::Minf<T>()), nt2::Inf<r_t>());
    ...
    for (uint32_t j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_EQUAL( nt2::ulp(a0),nt2::min(a0-nt2::prev(a0),nt2::next(a0)-a0));
     }
     
   }
} // end of test for real_

NT2_TEST_CASE_TPL ( ulp_unsigned_int__1_0,  NT2_UNSIGNED_TYPES)
{
  ...
  // specific values tests
  NT2_TEST_EQUAL(ulp(nt2::One<T>()), nt2::One<r_t>());
  NT2_TEST_EQUAL(ulp(nt2::Zero<T>()), nt2::One<r_t>());
  // random verifications
  static const uint32_t NR = NT2_NB_RANDOM_TEST;
  {
    ...
    for (uint32_t j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_EQUAL( nt2::ulp(a0),nt2::One<T>());
     }
     
   }
} // end of test for unsigned_int_

NT2_TEST_CASE_TPL ( ulp_signed_int__1_0,  NT2_INTEGRAL_SIGNED_TYPES)
{
  ...
  // specific values tests
  NT2_TEST_EQUAL(ulp(nt2::Mone<T>()), nt2::One<r_t>());
  NT2_TEST_EQUAL(ulp(nt2::One<T>()), nt2::One<r_t>());
  NT2_TEST_EQUAL(ulp(nt2::Zero<T>()), nt2::One<r_t>());
  // random verifications
  static const uint32_t NR = NT2_NB_RANDOM_TEST;
