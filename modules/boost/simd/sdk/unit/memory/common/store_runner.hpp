...
inline void masked_aligned_store_runner(bool offset = false)
{
  ...
  BOOST_SIMD_ALIGNED_STACK_BUFFER( ref , Type  , sz );
  ...
  if (!offset) aligned_store(v,&out[0],mask);
  else         aligned_store(v,&out[0],offset_dist,mask);
