...
NT2_TEST_CASE_TPL ( if_allbits_else_real__2_0,  BOOST_SIMD_SIMD_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(if_allbits_else(boost::simd::True<vT>(),boost::simd::One<vT>())[0], boost::simd::Nan<T>());
  NT2_TEST_EQUAL(if_allbits_else(boost::simd::False<vT>(),boost::simd::splat<vT>(1))[0], 1);
  NT2_TEST_EQUAL(if_allbits_else(boost::simd::False<vT>(),boost::simd::Zero<vT>())[0], 0 );
