...
#include <boost/simd/toolbox/standard/include/functions/abs.hpp>
    ...
    for(nt2::uint32_t j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_EQUAL( boost::simd::abs(a0),a0>0 ? T(a0) : T(-a0));
     }
     
   }
} // end of test for floating_

NT2_TEST_CASE_TPL ( abs_unsigned_int__1_0,  BOOST_SIMD_UNSIGNED_TYPES)
{
...
NT2_TEST_CASE_TPL ( abs_signed_int__1_0,  BOOST_SIMD_INTEGRAL_SIGNED_TYPES)
{
