...
NT2_TEST_CASE_TPL( digit_value, BOOST_SIMD_TYPES )
{
  ...
  for(std::size_t i=0; i< boost::simd::meta::cardinal_of<dst_t>::value;++i)
    NT2_TEST_EQUAL( (boost::simd::Fortyfive<dst_t>())[i], static_cast<T>( 45 ) );

  for(std::size_t i=0; i< boost::simd::meta::cardinal_of<dst_t>::value;++i)
    NT2_TEST_EQUAL( (boost::simd::Ninety<dst_t>())[i], static_cast<T>( 90 ) );

  for(std::size_t i=0; i< boost::simd::meta::cardinal_of<dst_t>::value;++i)
    NT2_TEST_EQUAL( (boost::simd::Hundred<dst_t>())[i], static_cast<T>( 100 ) );

  for(std::size_t i=0; i< boost::simd::meta::cardinal_of<dst_t>::value;++i)
    NT2_TEST_EQUAL( (boost::simd::Onehundredeighty<dst_t>())[i], static_cast<T>( 180 ) );

  for(std::size_t i=0; i< boost::simd::meta::cardinal_of<dst_t>::value;++i)
    NT2_TEST_EQUAL( (boost::simd::Thousand<dst_t>())[i], static_cast<T>( 1000 ) );
}
