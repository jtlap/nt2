...
NT2_TEST_CASE_TPL( unaligned_store,  BOOST_SIMD_SIMD_TYPES)
{
  ...
  unaligned_store_runner< T  , native<T,ext_t>   >();
  unaligned_store_runner< T  , pack<T>           >();
}
...
NT2_TEST_CASE_TPL( unaligned_store_offset,  BOOST_SIMD_SIMD_TYPES)
{
  ...
  unaligned_store_runner< T  , native<T,ext_t>   >(true);
  unaligned_store_runner< T  , pack<T>           >(true);
}
...
NT2_TEST_CASE( unaligned_store_sequence_pointer )
{
  ...
  unaligned_store_runner< foo, native<foo,ext_t> >();
  unaligned_store_runner< foo, native<foo,ext_t> >(true);
}
...
NT2_TEST_CASE( unaligned_store_sequence )
{
  ...
  seq_t v = unaligned_load<seq_t>(make_vector(&sdata[0], &fdata[0], &cdata[0]), 0);
  unaligned_store(v, make_vector(&ssdata[0], &sfdata[0], &scdata[0]));
