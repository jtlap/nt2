...
//  The result r always exactly fits in an simd vector of N elements
//       r =  zero<vT>
//       for(int i=0; i <N ; i++)
//       {
//         r+= rlh(a[i]*di < (2*i)%(N/2) > (b[i/(N/2)], b[i/(N/2)+2])
//             + ruh(a[i]*di < (2*i)/(N/2)+1 > (b[i/(N/2)], b[i/(N/2)+2]);
//       }
...
namespace boost { namespace simd {
  ...
  namespace details
  {
    ...
    template < size_t N, size_t I, class A, class B, class R, size_t START = 0u> struct do_compute
    {
    BOOST_FORCEINLINE void operator()(const A& a, const B & b, R & r) const
      {
        static const size_t F = (2*(I-1))%(N);
        static const size_t G = START+(I-1)/(N/2);
        r = fma(repeat_lower_half(a[I-1]), details::di<F>(b[G],b[G+2])
               , fma(repeat_upper_half(a[I-1]), details::di<F+1>(b[G],b[G+2])
                   , r)
               );
        do_compute<N, I-1, A, B, R, START>()(a, b, r);
      }
    };
    template < size_t N, class A, class B, class R, size_t START > struct do_compute <N, 0, A, B, R, START>
    {
...
namespace ext
{
  ...
  BOOST_SIMD_FUNCTOR_IMPLEMENTATION ( boost::simd::tag::mul_hn_
                                    ...
                                    , ((array_<simd_<floating_<A>,X>
                                      ...
                                      (mpl_integral_<scalar_<integer_<START>>>)
                                    )

  {
    typedef typename A::value_type  result_type;

    BOOST_FORCEINLINE  result_type operator()(const A& a, const B & b, const START&) const
    {
      BOOST_STATIC_ASSERT_MSG((P::value/4)*4 == P::value
                             , "B size must be divisible by 4" );
      static const size_t N = meta::cardinal_of<result_type>::value;
      result_type r = Zero<result_type>();
      details::do_compute<N, N, A, B, result_type, size_t(START::value)>()(a, b, r);
      return r;
    }
  };
