...
#include <boost/simd/matprod/functions/simd/common/mul_ntp.hpp>
...
NT2_TEST_CASE_TPL ( mul_n1,  BOOST_SIMD_SIMD_REAL_TYPES)
{
  ...
  using boost::simd::mul_ntp;
  using boost::simd::tag::mul_ntp_;
  ...
  {
    v.va[i] = boost::simd::enumerate<vT>(T(i+1), T(i));
  ...
  for(size_t c=0; c < P ; c++)
  {
    for(size_t l=0; l <card ; l++)
    {
      ...
      {
        vr.ta[l+card*c] +=t.ta[l+card*k]*v.ta[card*c+k];
      }
    }
  }
  vrref.va = vr.va;
  ...
  boost::array<vT, P >    r = mul_ntp(a, x);
