...
NT2_TEST_CASE_TPL ( is_not_equal_integer,  BOOST_SIMD_INTEGRAL_TYPES)
{
 ...
 NT2_TEST_TYPE_IS(r_t, wished_r_t);
  ...
  NT2_TEST_EQUAL(is_not_equal(boost::simd::One<T>(),boost::simd::Zero<T>()), r_t(true));
  NT2_TEST_EQUAL(is_not_equal(boost::simd::Zero<T>(), boost::simd::Zero<T>()), r_t(false));
...
NT2_TEST_CASE_TPL ( is_not_equal_real,  BOOST_SIMD_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(is_not_equal(boost::simd::Minf<T>(), boost::simd::Minf<T>()), r_t(false));
  NT2_TEST_EQUAL(is_not_equal(boost::simd::Nan<T>(), boost::simd::Nan<T>()), r_t(true));
  NT2_TEST_EQUAL(is_not_equal(boost::simd::One<T>(),boost::simd::Zero<T>()), r_t(true));
  NT2_TEST_EQUAL(is_not_equal(boost::simd::Zero<T>(), boost::simd::Zero<T>()), r_t(false));
...
NT2_TEST_CASE_TPL ( is_not_equall,  BOOST_SIMD_SIMD_TYPES)
{
  ...
  NT2_TEST_EQUAL(is_not_equal(boost::simd::True< boost::simd::logical<T> >(), boost::simd::True< boost::simd::logical<T> >()), r_t(false));
...
NT2_TEST_CASE ( is_not_equal_bool)
{
  using boost::simd::is_not_equal;
  using boost::simd::tag::is_not_equal_;
  typedef typename boost::dispatch::meta::call<is_not_equal_(bool, bool)>::type r_t;
  ...
  // specific values tests
  NT2_TEST_EQUAL(is_not_equal(true, false), true);
  NT2_TEST_EQUAL(is_not_equal(false, true), true);
  NT2_TEST_EQUAL(is_not_equal(true, true), false);
  NT2_TEST_EQUAL(is_not_equal(false, false), false);
}
