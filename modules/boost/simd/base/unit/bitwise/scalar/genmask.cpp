...
NT2_TEST_CASE_TPL ( genmask_real__1_0,  BOOST_SIMD_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(genmask(boost::simd::Inf<T>()), boost::simd::Allbits<r_t>());
  NT2_TEST_EQUAL(genmask(boost::simd::Minf<T>()), boost::simd::Allbits<r_t>());
  NT2_TEST_EQUAL(genmask(boost::simd::Nan<T>()), boost::simd::Allbits<r_t>());
#endif
...
NT2_TEST_CASE_TPL ( genmask_integer__1_0,  BOOST_SIMD_INTEGRAL_TYPES)
{
  ...
  // specific values tests
  NT2_TEST_EQUAL(genmask(boost::simd::One<T>()), boost::simd::Allbits<r_t>());
  NT2_TEST_EQUAL(genmask(boost::simd::Zero<T>()), boost::simd::Zero<r_t>());
