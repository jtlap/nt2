...
NT2_TEST_CASE_TPL ( rol_integer, BOOST_SIMD_SIMD_INTEGRAL_TYPES)
{
  ...
  NT2_TEST_TYPE_IS( typename boost::dispatch::meta::call<rol_(vT,ivT)>::type
                  , vT
                  );

  NT2_TEST_TYPE_IS( typename boost::dispatch::meta::call<rol_(vT,iT)>::type
                  , vT
                  );
  ...
  for(int i=0;i<w;++i)
    NT2_TEST_EQUAL(rol(One<vT>(),iT(i)), splat<vT>( T(T(1)<<i )) );
...
NT2_TEST_CASE_TPL ( rol_real, BOOST_SIMD_SIMD_REAL_TYPES)
{
  using boost::simd::rol;
  using boost::simd::tag::rol_;
  using boost::simd::native;
  ...
  for(int i=0;i<w;++i)
    NT2_TEST_EQUAL( rol( bitwise_cast<vT>(One<ivT>()),iT(i))
                  , bitwise_cast<vT>(splat<ivT>( iT(iT(1)<<(i)) ))
                  );
