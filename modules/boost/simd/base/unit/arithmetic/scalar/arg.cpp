...
NT2_TEST_CASE_TPL ( arg_real,  BOOST_SIMD_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(arg(boost::simd::Inf<T>()), boost::simd::Zero<r_t>());
  NT2_TEST_EQUAL(arg(boost::simd::Minf<T>()), boost::simd::Pi<r_t>());
  NT2_TEST_EQUAL(arg(boost::simd::Nan<T>()), boost::simd::Nan<r_t>());
#endif
