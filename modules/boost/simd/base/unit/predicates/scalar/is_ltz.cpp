...
NT2_TEST_CASE_TPL ( is_ltz_real__1_0,  BOOST_SIMD_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(is_ltz(boost::simd::Inf<T>()), r_t(false));
  NT2_TEST_EQUAL(is_ltz(boost::simd::Minf<T>()), r_t(true));
  NT2_TEST_EQUAL(is_ltz(boost::simd::Nan<T>()), r_t(false));
#endif
  NT2_TEST_EQUAL(is_ltz(-boost::simd::Zero<T>()), r_t(false));
  NT2_TEST_EQUAL(is_ltz(boost::simd::Half<T>()), r_t(false));
  NT2_TEST_EQUAL(is_ltz(boost::simd::Mone<T>()), r_t(true));
  NT2_TEST_EQUAL(is_ltz(boost::simd::One<T>()), r_t(false));
  NT2_TEST_EQUAL(is_ltz(boost::simd::Quarter<T>()), r_t(false));
...
NT2_TEST_CASE ( is_ltz_bool)
{
  using boost::simd::is_ltz;
  using boost::simd::tag::is_ltz_;
  typedef boost::dispatch::meta::call<is_ltz_(bool)>::type r_t;
