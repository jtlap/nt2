...
NT2_TEST_CASE_TPL ( is_nan_real__1_0,  BOOST_SIMD_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(is_nan(boost::simd::Inf<T>()), r_t(false));
  NT2_TEST_EQUAL(is_nan(boost::simd::Minf<T>()), r_t(false));
  NT2_TEST_EQUAL(is_nan(boost::simd::Nan<T>()), r_t(true));
#endif
  NT2_TEST_EQUAL(is_nan(-boost::simd::Zero<T>()), r_t(false));
  NT2_TEST_EQUAL(is_nan(boost::simd::Half<T>()), r_t(false));
  NT2_TEST_EQUAL(is_nan(boost::simd::Mone<T>()), r_t(false));
  NT2_TEST_EQUAL(is_nan(boost::simd::One<T>()), r_t(false));
  NT2_TEST_EQUAL(is_nan(boost::simd::Quarter<T>()), r_t(false));
  NT2_TEST_EQUAL(is_nan(boost::simd::Two<T>()), r_t(false));
  NT2_TEST_EQUAL(is_nan(boost::simd::Zero<T>()), r_t(false));
}
...
NT2_TEST_CASE ( is_nan_bool)
{
  ...
  // specific values tests
  NT2_TEST_EQUAL(is_nan(true), false);
  NT2_TEST_EQUAL(is_nan(false), false);
}
