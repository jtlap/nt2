...
NT2_TEST_CASE ( is_not_infinite_bool)
{
  using boost::simd::is_not_infinite;
  using boost::simd::tag::is_not_infinite_;
  typedef typename boost::dispatch::meta::call<is_not_infinite_(bool)>::type r_t;
  ...
  // specific values tests
  NT2_TEST_EQUAL(is_not_infinite(true), true);
  NT2_TEST_EQUAL(is_not_infinite(false), true);
}
