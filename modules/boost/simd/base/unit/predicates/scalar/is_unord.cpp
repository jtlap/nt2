...
NT2_TEST_CASE_TPL ( is_unord_real__2_0,  BOOST_SIMD_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(is_unord(boost::simd::Inf<T>(), boost::simd::Inf<T>()), r_t(false));
  NT2_TEST_EQUAL(is_unord(boost::simd::Minf<T>(), boost::simd::Minf<T>()), r_t(false));
  NT2_TEST_EQUAL(is_unord(boost::simd::Nan<T>(), boost::simd::Nan<T>()), r_t(true));
#endif
  NT2_TEST_EQUAL(is_unord(-boost::simd::Zero<T>(), -boost::simd::Zero<T>()), r_t(false));
  NT2_TEST_EQUAL(is_unord(boost::simd::Half<T>(), boost::simd::Half<T>()), r_t(false));
  NT2_TEST_EQUAL(is_unord(boost::simd::Mone<T>(), boost::simd::Mone<T>()), r_t(false));
  NT2_TEST_EQUAL(is_unord(boost::simd::One<T>(), boost::simd::One<T>()), r_t(false));
  NT2_TEST_EQUAL(is_unord(boost::simd::Quarter<T>(), boost::simd::Quarter<T>()), r_t(false));
...
NT2_TEST_CASE ( is_unord_bool)
{
  using boost::simd::is_unord;
  using boost::simd::tag::is_unord_;
  typedef boost::dispatch::meta::call<is_unord_(bool, bool)>::type r_t;
