...
NT2_TEST_CASE_TPL ( cummax_real,  BOOST_SIMD_SIMD_REAL_TYPES)
{
  ...
  NT2_TEST_EQUAL(cummax(boost::simd::Inf<vT>()) ,  byhands(boost::simd::Inf<vT>()));
  NT2_TEST_EQUAL(cummax(boost::simd::Minf<vT>()),  byhands(boost::simd::Minf<vT>()));
  NT2_TEST_EQUAL(cummax(boost::simd::Nan<vT>()) ,  byhands(boost::simd::Nan<vT>()));
#endif
