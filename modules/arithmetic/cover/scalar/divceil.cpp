    ...
    for(nt2::uint32_t j =0; j < NR; ++j )
      {
        std::cout << "for params "
                  ...
                  << ", a1 = "<< u_t(a1 = tab_a1[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::divceil(a0,a1),r_t(a1 ? nt2::ceil((1.0*a0)/a1) : nt2::Valmax<r_t>()),0);
        ulp0=nt2::max(ulpd,ulp0);
     }
     std::cout << "max ulp found is: " << ulp0 << std::endl;
   }
    ...
    for(nt2::uint32_t j =0; j < NR; ++j )
      {
        std::cout << "for params "
                  ...
                  << ", a1 = "<< u_t(a1 = tab_a1[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::divceil(a0,a1),r_t(a1 ? nt2::ceil((1.0*a0)/a1) : ((a0>0)? nt2::Valmax<r_t>(): (a0<0) ? nt2::Valmin<r_t>() : 0)),0);
        ulp0=nt2::max(ulpd,ulp0);
     }
     std::cout << "max ulp found is: " << ulp0 << std::endl;
   }
