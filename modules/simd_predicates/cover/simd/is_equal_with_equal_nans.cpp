...
BOOST_SIMD_TEST_CASE_TPL ( is_equal_with_equal_nans_real__2_0,  BOOST_SIMD_REAL_TYPES)
{
  ...
  typedef typename boost::simd::meta::call<is_equal_with_equal_nans_(vT,vT)>::type r_t;
  typedef typename boost::simd::meta::call<is_equal_with_equal_nans_(T,T)>::type sr_t;
    ...
    for(boost::simd::uint32_t j = 0; j < NR/cardinal_of<n_t>::value; j++)
      {
        ...
        for(int i = 0; i< cardinal_of<n_t>::value; i++)
        {
          ...
          BOOST_SIMD_TEST_EQUAL( v[i]!=0,ssr_t(boost::simd::is_equal_with_equal_nans (tab_a0[k],tab_a1[k])));
        }
      }
