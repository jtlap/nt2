...
NT2_TEST_CASE( hierarchy )
{
  ...
  NT2_TEST_EXPR_TYPE( (nt2::tag::Zero() )
  ...
  NT2_TEST_EXPR_TYPE( (nt2::tag::Zero() )
                    ...
                    , (nt2::ext::pure_constant_< nt2::tag::Zero >
                      )
                    );

  NT2_TEST_EXPR_TYPE( (nt2::tag::Zero() )
                    , (nth_hierarchy<_,int_<2> >)
                    , (nt2::ext::constant_< nt2::tag::Zero >
                      )
                    );

  NT2_TEST_EXPR_TYPE( (nt2::tag::Zero() )
                    , (nth_hierarchy<_,int_<3> >)
                    , (nt2::ext::elementwise_<nt2::tag::Zero>)
                    );

  NT2_TEST_EXPR_TYPE( (nt2::tag::Zero() )
                    , (nth_hierarchy<_,int_<4> >)
                    , (nt2::ext::unspecified_<nt2::tag::Zero>)
                    );
}
...
NT2_TEST_CASE_TPL( _0d_typed, NT2_TYPES )
{
  ...
  nt2::table< T > x1 = nt2::zeros( as_<T>() );

  NT2_TEST_EXPR_TYPE( nt2::zeros( as_<T>() ), (value_type_<_>), (T) );
  NT2_TEST_EXPR_TYPE( x1, (value_type_<_>), (T) );
  NT2_TEST_EQUAL( nt2::extent(x1), nt2::of_size(1) );
  NT2_TEST_EQUAL( x1(1), nt2::Zero<T>() );
}
...
NT2_TEST_CASE( untyped_square )
{
  ...
  for(int i=1;i<= 3;++i)
    for(int j=1;j<= 3;++j)
      ref(i,j) = nt2::Zero<double>();

  nt2::table< double > x0 = nt2::zeros(3);

  NT2_TEST_EXPR_TYPE( (nt2::zeros( 3 )), (value_type_<_>), (double) );
...
NT2_TEST_CASE( nd_untyped )
{
  ...
  nt2::table< double > x1 = nt2::zeros(8, 4);

  NT2_TEST_EXPR_TYPE( (nt2::zeros(8,4)), (value_type_<_>), (double) );
  NT2_TEST_EQUAL( nt2::extent(x1), nt2::of_size(8,4) );
  NT2_TEST_EQUAL(x1,ref);

  ref.resize( nt2::of_size(1,4) );
    for(int j=1;j<= 4;++j)
      ref(1,j) = nt2::Zero<double>();

  nt2::table< double > x2 = nt2::zeros(1, 4);
  NT2_TEST_EQUAL( nt2::extent(x2), nt2::of_size(1,4) );
  NT2_TEST_EQUAL(x2,ref);

  ref.resize( nt2::of_size(7,1) );
    for(int i=1;i<= 7;++i)
      ref(i,1) = nt2::Zero<double>();

  nt2::table< double > x3 = nt2::zeros(7,1);
...
NT2_TEST_CASE_TPL( nd_typed, NT2_TYPES )
{
  ...
  nt2::table< T > x1 = nt2::zeros(nt2::of_size(8, 8), as_<T>() );

  NT2_TEST_EXPR_TYPE( (nt2::zeros( nt2::of_size(8, 8), as_<T>() )), (value_type_<_>), (T) );
  NT2_TEST_EQUAL(x1,ref);

  nt2::table< T > x2 = nt2::zeros(8, 8, as_<T>() );
  NT2_TEST_EQUAL(x2,ref);

  nt2::table< T > x3 = nt2::zeros(8, as_<T>() );
  NT2_TEST_EQUAL(x3,ref);
}
...
NT2_TEST_CASE( expr )
{
  ...
  nt2::table< double > x1 = nt2::zeros( t1 );

  NT2_TEST_EXPR_TYPE( (nt2::zeros( t1 )), (value_type_<_>), (double) );
  NT2_TEST_EQUAL(x1,ref);
  ...
  nt2::table< double > x2 = nt2::zeros( t2 );
  NT2_TEST_EQUAL(x2,ref);

  NT2_TEST_ASSERT( x1 = nt2::zeros(x2) );
}
...
NT2_TEST_CASE_TPL( typed_expr, NT2_TYPES )
{
  ...
  nt2::table< T > x1 = nt2::zeros( t1, as_<T>() );

  NT2_TEST_EXPR_TYPE( (nt2::zeros( t1, as_<T>() )), (value_type_<_>), (T) );
  NT2_TEST_EQUAL(x1,ref);
  ...
  nt2::table< T > x2 = nt2::zeros( t2, as_<T>() );
  NT2_TEST_EQUAL(x2,ref);

  NT2_TEST_ASSERT( x1 = nt2::zeros(x2, as_<T>()) );
}
