 ...
 ******************************************************************************/
#define NT2_UNIT_MODULE "nt2 container bench"
...
template<class T> struct container_1D_test
{
  ...
  container_1D_test ( std::size_t s0 )
                    : data(nt2::of_size(s0))
                    , data2(nt2::of_size(s0))
                    , s0_(s0)
  {}

  void operator()()
  {
    for(std::size_t i = 0; i < s0_; ++i) data[i] = data2[i];
  }
...
{
  std_1D_test(std::size_t n) : data(n),data2(n) {}
...
{
  std::cout << "Size\tnt2::container\tstd::vector\toverhead\%\n";
