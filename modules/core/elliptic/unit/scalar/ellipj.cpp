...
NT2_TEST_CASE_TPL ( ellipj_real__1_0,  NT2_REAL_TYPES)
{
  ...
  NT2_TEST_TYPE_IS( (typename boost::dispatch::meta::call<ellipj_(T, T)>::type)
                  , (tuple<T,T,T>)
                  );
  NT2_TEST_TYPE_IS( (typename boost::dispatch::meta::call<ellipj_(T, T, T)>::type)
                  , (tuple<T,T,T>)
                  );
  NT2_TEST_TYPE_IS( (typename boost::dispatch::meta::call<ellipj_(T, T, T, T&)>::type)
                  , (tuple<T,T>)
                  );
  NT2_TEST_TYPE_IS( (typename boost::dispatch::meta::call<ellipj_(T, T, T, T&, T&)>::type)
                  , (T)
                  );
  NT2_TEST_TYPE_IS( (typename boost::dispatch::meta::call<ellipj_(T, T, T, T&, T&, T&)>::type)
                  , (void)
                  );
  ...
  for(size_t i=1; i <= 3; ++i)
  {
    for(size_t j=1; j <= N; ++j)
    {
      ellipj(ui[i-1], m[j-1],nt2::Eps<T>(), s, c, d);
      NT2_TEST_ULP_EQUAL(s, sni(i, j), 1);
      NT2_TEST_ULP_EQUAL(c, cni(i, j), 1);
      NT2_TEST_ULP_EQUAL(d, dni(i, j), 1);
    }
  }
#endif
  ...
  for(size_t i=1; i <= 3; ++i)
  {
    for(size_t j=1; j <= N; ++j)
     {
       s = ellipj(ui[i-1], m[j-1],nt2::Eps<T>(), c, d);
       NT2_TEST_ULP_EQUAL(s, sni(i, j), 1);
       NT2_TEST_ULP_EQUAL(c, cni(i, j), 1);
       NT2_TEST_ULP_EQUAL(d, dni(i, j), 1);
     }
  ...
  for(size_t i=1; i <= 3; ++i)
  {
    for(size_t j=1; j <= N; ++j)
    {
      boost::fusion::vector_tie(s, c) = ellipj(ui[i-1], m[j-1],nt2::Eps<T>(), d);
      NT2_TEST_ULP_EQUAL(s, sni(i, j), 1);
      NT2_TEST_ULP_EQUAL(c, cni(i, j), 1);
      NT2_TEST_ULP_EQUAL(d, dni(i, j), 1);
    }
  ...
  for(size_t i=1; i <= 3; ++i)
  {
    for(size_t j=1; j <= N; ++j)
    {
      boost::fusion::vector_tie(s, c, d) = ellipj(ui[i-1], m[j-1],nt2::Eps<T>());
      NT2_TEST_ULP_EQUAL(s, sni(i, j), 1);
      NT2_TEST_ULP_EQUAL(c, cni(i, j), 1);
      NT2_TEST_ULP_EQUAL(d, dni(i, j), 1);
    }
  ...
  for(size_t i=1; i <= 3; ++i)
  {
    for(size_t j=1; j <= N; ++j)
    {
      boost::fusion::tuple<T, T, T> t = ellipj(ui[i-1], m[j-1]);
      NT2_TEST_ULP_EQUAL(boost::fusion::at_c<0>(t), sni(i, j), 1);
      NT2_TEST_ULP_EQUAL(boost::fusion::at_c<1>(t), cni(i, j), 1);
      NT2_TEST_ULP_EQUAL(boost::fusion::at_c<2>(t), dni(i, j), 1);
    }
  }
#endif
