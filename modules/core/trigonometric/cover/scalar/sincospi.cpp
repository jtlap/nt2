...
//==============================================================================
// cover for functor sincospi in scalar mode
...
NT2_TEST_CASE_TPL(sincospi_0,  NT2_SIMD_REAL_TYPES)
{
  using nt2::unit::args;
  const std::size_t NR = args("samples", NT2_NB_RANDOM_TEST);
  const double ulpd = args("ulpd",  1.0);

  const T min = args("min", T(-40));
  const T max = args("max", T(40));
  ...
  for(std::size_t i=0; i!=NR; ++i)
    ref[i] = std::make_pair(nt2::sinpi(a0[i]), nt2::cospi(a0[i]));

  NT2_COVER_ULP_EQUAL(nt2::tag::sincospi_, ((T, a0)), ref, ulpd);
}
