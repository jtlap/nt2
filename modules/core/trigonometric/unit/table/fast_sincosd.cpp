...
NT2_TEST_CASE_TPL(fast_sincosd_table, NT2_REAL_TYPES)
{
  using nt2::fast_sincosd;
  using nt2::table;
  typedef table<T> tab_t;
  static const int nb = 60;
  tab_t a = nt2::linspace(T(0), T(45), nb);
  tab_t  s(nt2::of_size(1, nb)), c(nt2::of_size(1, nb));
  {
    fast_sincosd(a, s, c);
    NT2_TEST_ULP_EQUAL(s, nt2::sind(a), 1.0);
    NT2_TEST_ULP_EQUAL(c, nt2::cosd(a), 1.0);
  }
