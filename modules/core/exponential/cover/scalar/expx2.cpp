...
//==============================================================================
// cover for functor expx2 in scalar mode
...
extern "C" { long double cephes_expx2l(long double, int); }
...
NT2_TEST_CASE_TPL(expx2_0,  NT2_SIMD_REAL_TYPES)
{
  ...
  {
    ref1[i] = cephes_expx2l(a0[i], 1);
    ref2[i] = cephes_expx2l(a0[i], -1);
    a1[i] = T(-1.0);
  }

  std::cout << "Argument samples #0 chosen in range: [" << min << ",  " << max << "]" << std::endl;
  std::cout << "Argument #1 is 1" << std::endl;
  NT2_COVER_ULP_EQUAL(nt2::tag::expx2_, ((T, a0)), ref1, ulpd);
  std::cout << "Argument samples #0 chosen in range: [" << min << ",  " << max << "]" << std::endl;
  std::cout << "Argument #1 is -1" << std::endl;
