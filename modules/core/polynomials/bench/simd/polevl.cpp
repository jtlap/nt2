...
template<typename T> NT2_EXPERIMENT(polevl_bench)
{
  public:
  typedef typename boost::simd::meta::scalar_of<T>::type base_t;

  polevl_bench(std::size_t s) : NT2_EXPRIMENT_CTOR(1,"cycles/elements"), size(s)
  {
  ...
  virtual void run() const
  {
    for(int i=0;i<size;i+=T::static_size)
      boost::simd::store(nt2::polevl(boost::simd::load<T>(&in[i]), coeff),&out[i]);
  }
...
NT2_RUN_EXPERIMENT_TPL( polevl_bench, (vf_t)(vd_t), (1024) );
