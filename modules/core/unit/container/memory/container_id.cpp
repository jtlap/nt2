...
NT2_TEST_CASE_TPL( container_buffer_dimensions, DIMS )
{
  ...
  NT2_TEST_EQUAL
  ((dimensions_of< container<table_, id_<1337>, double, settings(T)> >::value)
  , T::static_size
  );
}
...
NT2_TEST_CASE_TPL( container_values, NT2_TYPES )
{
  ...
  NT2_TEST_EXPR_TYPE( (container <table_, id_<42>, T, settings()>())
                    , value_of<_>
                    , T
                    );
}
...
NT2_TEST_CASE_TPL( container_models, NT2_TYPES )
{
  ...
  NT2_TEST_EXPR_TYPE
  ( (container<table_, id_<999>, int, settings()>())
  , (apply_model<_,T>)
  , (container<table_, id_<999>, T, settings()>)
  );
}
...
NT2_TEST_CASE_TPL( container_dynamic_default_ctor, DIMS)
{
  ...
  container<table_,id_<777>,double,settings(T)> b;
...
NT2_TEST_CASE_TPL( container_static_default_ctor, NT2_TYPES)
{
  ...
  typedef container<table_,id_<69>,T,settings(of_size_<32,7>)> type;
  type b;
  ...
  for(typename type::difference_type j=b.outer_lower();j<=b.outer_upper();++j)
    for(typename type::difference_type i=b.inner_lower();i<=b.inner_upper();++i)
...
NT2_TEST_CASE_TPL( container_automatic_static_default_ctor, NT2_TYPES)
{
  ...
  typedef container<table_,id_<43>,T,settings(of_size_<32,7>, automatic_)> type;
  type b;
...
NT2_TEST_CASE_TPL( container_size_ctor, NT2_TYPES)
{
  ...
  typedef container<table_,id_<404>,T,settings()> type;
  type b( of_size(32,3) );
...
NT2_TEST_CASE_TPL( container_copy_ctor, NT2_TYPES)
{
  ...
  typedef container<table_,id_<1338>,T,settings()> type;
  type b( of_size(32,3) );
  ...
  NT2_TEST_EQUAL(x.inner_size() ,  32   );
  NT2_TEST_EQUAL(x.outer_size() ,  3    );
  NT2_TEST_EQUAL(x.lower()      ,  1    );
  NT2_TEST_EQUAL(x.inner_lower(),  1    );
  NT2_TEST_EQUAL(x.outer_lower(),  1    );
  NT2_TEST_EQUAL(x.upper()      ,  32*3 );
  NT2_TEST_EQUAL(x.inner_upper(),  32   );
  NT2_TEST_EQUAL(x.outer_upper(),  3    );
  NT2_TEST( (x.extent() == of_size(32,3)) );

  for(typename type::difference_type j=x.outer_lower();j<=x.outer_upper();++j)
    for(typename type::difference_type i=x.inner_lower();i<=x.inner_upper();++i)
...
NT2_TEST_CASE_TPL( container_swap, NT2_TYPES)
{
  ...
  typedef container<table_,id_<421>,T,settings()> type;
  type b( of_size(32,3) );
  type x( of_size(32,2) );
