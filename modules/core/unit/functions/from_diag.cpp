...
NT2_TEST_CASE_TPL( from_diag_offset, (float))//NT2_TYPES )
{
  nt2::table<T> x,y( nt2::of_size(1,5) );
  ...
  disp("x ", x); 
  NT2_TEST_EQUAL( nt2::ndims(x) , 2             );
  NT2_TEST_EQUAL( nt2::size(x,1), nt2::length(y)+1);
  NT2_TEST_EQUAL( nt2::size(x,2), nt2::length(y)+1);

  for(int i=1;i<=nt2::length(x);i++) NT2_TEST_EQUAL( x(i,i+1), y(i) );
}
