...
// }
 ...
 NT2_TEST_CASE( ric_nd_untyped )
 {
   ...
   for(int i=0;i<3;++i) for(int j=0;j<3;++j)NT2_TEST_EQUAL( double(i-1), double(x1(i, j)) );
   for(int i=0;i< 3;++i) {
     for(int j=0;j< 3;++j){
   ...
   for(int i=1;i<=8;++i) for(int j=1;j<=6;++j)NT2_TEST_EQUAL( double(i-1), double(x6(i, j)) );
...
NT2_TEST_CASE( ric_expr )
{
  ...
  for(int i=1;i<=4;++i) for(int j=1;j<=5;++j) NT2_TEST_EQUAL(  double(i-1), double(x1(i, j)) );
...
NT2_TEST_CASE_TPL( ric_typed_expr, NT2_TYPES )
{
  ...
  for(int i=1;i<=3;++i) for(int j=1;j<=4;++j) NT2_TEST_EQUAL( T(i-1), T(x1(i, j)));
  nt2::table<int> a( nt2::of_size(4,5) );
  nt2::table<T> x2 = nt2::ric( nt2::size(a), nt2::meta::as_<T>() );
  for(int i=1;i<=4;++i) for(int j=1;j<=5;++j) NT2_TEST_EQUAL( T(i-1), T(x2(i, j)));
