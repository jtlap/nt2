...
//==============================================================================
// cover for functor spence in simd mode
...
NT2_TEST_CASE_TPL(spence_0,  NT2_SIMD_REAL_TYPES)
{
  ...
  const T min = args("min", T(-1e30));
  const T max = args("max", T(1e30));
  std::cout << "Argument samples #0 chosen in range: [" << min << ",  " << max << "]" << std::endl;
  NT2_CREATE_BUF(a0,T, NR, min, max);

  std::vector<T> ref(NR);
  for(std::size_t i=0; i!=NR; ++i)
    ref[i] = nt2::spence(a0[i]);

  NT2_COVER_ULP_EQUAL(nt2::tag::spence_, ((vT, a0)), ref, ulpd);
