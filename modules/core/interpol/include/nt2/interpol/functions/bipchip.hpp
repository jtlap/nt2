...
//==============================================================================
#ifndef NT2_INTERPOL_FUNCTIONS_BIPCHIP_HPP_INCLUDED
#define NT2_INTERPOL_FUNCTIONS_BIPCHIP_HPP_INCLUDED
...
namespace nt2 { namespace tag
  {
   /*!
     @brief bipchip generic tag

     Represents the bipchip function in generic contexts.

     @par Models:
        Hierarchy
   **/
    struct bipchip_ : ext::unspecified_<bipchip_>
    {
      /// @brief Parent hierarchy
      typedef ext::unspecified_<bipchip_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_bipchip_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
   template<class Site>
   BOOST_FORCEINLINE generic_dispatcher<tag::bipchip_, Site> dispatching_bipchip_(adl_helper, boost::dispatch::meta::unknown_<Site>, ...)
   {
     return generic_dispatcher<tag::bipchip_, Site>();
   }
   template<class... Args>
   struct impl_bipchip_;
  }
  /*!
    one dimensional bipchip interpolation
    \par
    ...
    @code
    auto r = bipchip (x, y, z, xi, yi)
    auto r = bipchip (z, xi, yi)
    auto r = bipchip (z{, n])
    auto r = bipchip (z, xi, yi, extrapval)
    auto r = bipchip (x, y, z, xi, yi, extrapval)
    @endcode

     Two-dimensional interpolation return the Piecewise Cubic Hermite Interpolating Polynomial.
  ...
  **/
  NT2_FUNCTION_IMPLEMENTATION(tag::bipchip_, bipchip, 6)
  /// @overload
  NT2_FUNCTION_IMPLEMENTATION(tag::bipchip_, bipchip, 5)
  /// @overload
  NT2_FUNCTION_IMPLEMENTATION(tag::bipchip_, bipchip, 4)
  /// @overload
  NT2_FUNCTION_IMPLEMENTATION(tag::bipchip_, bipchip, 3)
  /// @overload
  NT2_FUNCTION_IMPLEMENTATION(tag::bipchip_, bipchip, 2)
  /// @overload
  NT2_FUNCTION_IMPLEMENTATION(tag::bipchip_, bipchip, 1)
  /// @overload
}
...
namespace nt2 { namespace ext
{
  ...
  struct  size_of<tag::bipchip_, Domain, 1, Expr>
  {
  ...
  /// INTERNAL ONLY
  template<class Domain, class Expr>
  struct  size_of<tag::bipchip_, Domain, 2, Expr>
  {
    typedef _2D                               result_type;
    BOOST_FORCEINLINE result_type operator()(Expr& e) const
    {
  ...
  struct  size_of<tag::bipchip_, Domain, N, Expr>
  {
    typedef _2D                               result_type;
