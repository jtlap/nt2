...
//==============================================================================
#ifndef NT2_LINALG_FUNCTIONS_BLAS_TPMV_HPP_INCLUDED
#define NT2_LINALG_FUNCTIONS_BLAS_TPMV_HPP_INCLUDED

/*!
  @file
  @brief Defines and implements blas_tpmv function
**/
...
namespace nt2
{
  namespace tag
  {
    /// @brief Defines blas_tpmv function tag
    struct blas_tpmv_ : ext::abstract_<blas_tpmv_>
    {
      /// INTERNAL ONLY
      typedef ext::abstract_<blas_tpmv_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_blas_tpmv_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
    template<class Site>
    BOOST_FORCEINLINE generic_dispatcher<tag::blas_tpmv_, Site> dispatching_blas_tpmv_(adl_helper, boost::dispatch::meta::unknown_<Site>, ...)
    {
      return generic_dispatcher<tag::blas_tpmv_, Site>();
    }
    template<class... Args>
    struct impl_blas_tpmv_;
  }

  /*!
    @brief tpmv  -  perform  one  of the matrix-vector operations
    x = mtimes(a, x)
    x = mtimes(trans(a), x)
    x = mtimes(ctrans(a), x)
    ...
    @param uplo   char
     ...
     @param diag   - char
      ...
      @param a  container of dimension nxn in packed form
  ...
  **/
  NT2_FUNCTION_IMPLEMENTATION_TPL (tag::blas_tpmv_, blas_tpmv
