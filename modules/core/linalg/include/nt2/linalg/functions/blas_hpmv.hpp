...
//==============================================================================
#ifndef NT2_LINALG_FUNCTIONS_BLAS_HPMV_HPP_INCLUDED
#define NT2_LINALG_FUNCTIONS_BLAS_HPMV_HPP_INCLUDED

/*!
  @file
  @brief Defines and implements blas_hpmv function
**/
...
namespace nt2
{
  namespace tag
  {
    /// @brief Defines blas_hpmv function tag
    struct blas_hpmv_ : ext::abstract_<blas_hpmv_>
    {
      /// INTERNAL ONLY
      typedef ext::abstract_<blas_hpmv_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_blas_hpmv_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
    template<class Site>
    BOOST_FORCEINLINE generic_dispatcher<tag::blas_hpmv_, Site> dispatching_blas_hpmv_(adl_helper, boost::dispatch::meta::unknown_<Site>, ...)
    {
      return generic_dispatcher<tag::blas_hpmv_, Site>();
    }
    template<class... Args>
    struct impl_blas_hpmv_;
  }

  /*!
    @brief      hpmv  performs  the  matrix-vector  operation y := alpha*ap*x + beta*y,
       where alpha and beta are scalars, x and y are n element vectors  and  ap
       is an n by n PACKED hermitian matrix.
  ...
  **/
  NT2_FUNCTION_IMPLEMENTATION_TPL (tag::blas_hpmv_, blas_hpmv
