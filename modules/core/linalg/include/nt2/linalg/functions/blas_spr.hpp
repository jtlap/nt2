...
//==============================================================================
#ifndef NT2_LINALG_FUNCTIONS_BLAS_SPR_HPP_INCLUDED
#define NT2_LINALG_FUNCTIONS_BLAS_SPR_HPP_INCLUDED

/*!
  @file
  @brief Defines and implements blas_spr function
**/
...
namespace nt2
{
  namespace tag
  {
    /// @brief Defines blas_spr function tag
    struct blas_spr_ : ext::abstract_<blas_spr_>
    {
      /// INTERNAL ONLY
      typedef ext::abstract_<blas_spr_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_blas_spr_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
    template<class Site>
    BOOST_FORCEINLINE generic_dispatcher<tag::blas_spr_, Site> dispatching_blas_spr_(adl_helper, boost::dispatch::meta::unknown_<Site>, ...)
    {
      return generic_dispatcher<tag::blas_spr_, Site>();
    }
    template<class... Args>
    struct impl_blas_spr_;
  }

  /*!
    @brief spr   performs the symmetric rank 1 operation with packed matrix

          a += alpha*x*ctrans(x)

       wspre alpha is a real scalar, x is an n element vector and a is an n by
       n symmetric matrix.

      uplo   - char
              on entry, uplo specifies whetspr the upper or  lower  triangular
  ...
  **/
  NT2_FUNCTION_IMPLEMENTATION_TPL (tag::blas_spr_, blas_spr
