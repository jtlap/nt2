...
//==============================================================================
#ifndef NT2_LINALG_FUNCTIONS_BLAS_TRMV_HPP_INCLUDED
#define NT2_LINALG_FUNCTIONS_BLAS_TRMV_HPP_INCLUDED

/*!
  @file
  @brief Defines and implements blas_trmv function
**/
...
namespace nt2
{
  namespace tag
  {
    /// @brief Defines blas_trmv function tag
    struct blas_trmv_ : ext::abstract_<blas_trmv_>
    {
      /// INTERNAL ONLY
      typedef ext::abstract_<blas_trmv_> parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_BODY( dispatching_blas_trmv_( ext::adl_helper(), static_cast<Args&&>(args)... ) )
    };
  }
  namespace ext
  {
    template<class Site>
    BOOST_FORCEINLINE generic_dispatcher<tag::blas_trmv_, Site> dispatching_blas_trmv_(adl_helper, boost::dispatch::meta::unknown_<Site>, ...)
    {
      return generic_dispatcher<tag::blas_trmv_, Site>();
    }
    template<class... Args>
    struct impl_blas_trmv_;
  }

  /*!
    @brief trmv  -  perform  one  of the matrix-vector operations
    x = mtimes(a, x)
    x = mtimes(trans(a), x)
    x = mtimes(ctrans(a), x)
  ...
  **/
  NT2_FUNCTION_IMPLEMENTATION_TPL (tag::blas_trmv_, blas_trmv
