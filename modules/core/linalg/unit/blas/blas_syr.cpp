...
//==============================================================================
#include <nt2/include/functions/blas_syr.hpp>
...
NT2_TEST_CASE_TPL(blas_syr, NT2_REAL_TYPES )
{
  using nt2::blas_syr;
  using nt2::reshape;
  using nt2::_;
  ...
  using nt2::average;
  {
    ...
    blas_syr('U', alpha, boost::proto::value(x), 1, boost::proto::value(a));
    NT2_TEST_ULP_EQUAL(triu(a), triu(z), 0.5);
    a = a1;
    z = alpha*mtimes(x, trans(x))+a;
    blas_syr('L', alpha, boost::proto::value(x), 1, boost::proto::value(a));
...

  {
    ...
    nt2::table<cT> a = average(b, trans(b));
    nt2::table<cT> a1 = a;
    nt2::table<cT> x = reshape(linspace(cT(0, 1), cT(2, 2), 2), 2, 1);
    cT alpha = cT(2, 3);
    nt2::table<cT> z = alpha*mtimes(x, trans(x))+a;
    blas_syr('U', alpha, boost::proto::value(x), 1, boost::proto::value(a));
    NT2_DISPLAY(a);
    NT2_TEST_ULP_EQUAL(triu(a), triu(z), 0.5);
    a = a1;
