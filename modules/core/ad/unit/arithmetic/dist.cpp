...
auto test(const T1 & x, const T2 & y) -> decltype(dist(x,y))
{
  return dist(x,y);
}
...
NT2_TEST_CASE_TPL ( dist,  NT2_REAL_TYPES)
{
  T x = 2, y = 4;
  {
    valder<T> vx(2, 1), vy(4, 0); //take x derivative
    valder<T> r1 = test(vx, vy);
    NT2_TEST_EQUAL(r1.der(), -1);
    NT2_TEST_EQUAL(r1.der(), test(vx, y).der());
  }
   {
    valder<T> vx(2, 0), vy(4, 1); //take y derivative
    valder<T> r1 = test(vx, vy);
    NT2_TEST_EQUAL(r1.der(), 1);
