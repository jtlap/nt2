...
auto test(const L & l, const T1 & x) -> decltype(if_else_zero(l, x))
{
  return if_else_zero(l, x);
}

using nt2::ad::valder;

NT2_TEST_CASE_TPL ( if_else_zero,  NT2_REAL_TYPES)
{
  {
    bool l =  true;
    valder<T> vx(2, 1);
  ...
  }
  {
    ...
    valder<T> vx(2, 1);
    valder<T> r1 = test(l, vx);
    NT2_TEST_EQUAL(r1.der(), nt2::Zero<T>());
  }  {
    T l = nt2::One<T>();
    valder<T> vx(2, 1);
    valder<T> r1 = test(l, vx);
