...
auto test(const L & l, const T1 & x, const T2 & y) -> decltype(if_else(l, x, y))
{
  return if_else(l, x, y);
}

using nt2::ad::valder;

NT2_TEST_CASE_TPL ( if_else,  NT2_REAL_TYPES)
{
  T x = 2, y = 3;
  {
    bool l = true;
    valder<T> vx(2, 1), vy(3, 1);
  ...
  {
    bool l = false;
    valder<T> vy(3, 1), vx(2, 1);
    valder<T> r1 = test(l, vx, vy);
    valder<T> r2 = test(l, vx,  y);
    valder<T> r3 = test(l,  x, vy);
    NT2_TEST_EQUAL(r1.der(), vy.der());
