...
{
  template<class T, class V> class integ_impl
  {
  public :
    typedef T                                              input_t;
    typedef V                                              value_t;
    typedef typename meta::as_real<value_t>::type           real_t;
    typedef details::integration_settings<T, V, tag::integ_>   o_t;
...
//         wpts_ = wpts_(d);
  ...
  private:
    ...
    template < class FUNC >
    value_t integstep(const FUNC & f, const input_t & a,  const input_t & b,
...
namespace nt2 { namespace ext
{
    ...
    )
  {
    ...
    {
      details::integ_impl<input_type, value_type> q;
      q.compute(f, x, o);
