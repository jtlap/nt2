  ...
  public :
   ...
   private :
    ...
    value_t compute(const  FUNC &f, const real_t& a, const real_t& b)
    {
      ...
      const real_t cs[] = {T(.942882415695480), T(nt2::sqrt(2/3)),
                           T(.641853342345781),
                           T(1.0/nt2::sqrt(5.0)), T(.236383199662150)};
      ...
      value_t Q1 = (h/T(6))*nt2::dot(lobatto(),y(nt2::_(1,4,13)));
      value_t Q2 = (h/T(1470))*nt2::dot(kronrod(), y(nt2::_(1,2,13)));
                      ...
                      const value_t &fa, const value_t& fb)
    {
      ...
      real_t alpha = nt2::sqrt(T(2)/T(3));
      real_t beta = T(1)/nt2::sqrt(T(5));
      T cx[] = {-alpha, -beta, Zero<T>(), beta, alpha};
      rtab_t x(of_size(1, 5), &cx[0], &cx[5]);
      x = c+h*x;
      tab_t y = f(x);
      fcnt_ += 5;
      if (fcnt_ > maxfcnt_)// Maximum function count exceeded; singularity likely.
      {
        setwarn(2); return h*(fa+fb);
      }
      rtab_t x1 = nt2::cath(nt2::cath(a, x), b); x = x1;
      tab_t y1 = nt2::cath(nt2::cath(fa, y), fb);y =  y1;
      value_t Q1 = (h/T(6))*nt2::dot(lobatto(), y(_(1, 2, 7))); // Four point Lobatto quadrature.
      value_t Q2 = (h/T(1470))*nt2::dot(kronrod(), y);// Seven point Kronrod refinement.
      value_t Q = Q2;
      if (nt2::is_invalid(Q))// Infinite or Not-a-Number function value encountered.
      {
        setwarn(3); return Q;
      }
      //  Check accuracy of integral over this subinterval.
      if (nt2::abs(Q1 - Q2) <= tol_)
      {
        setwarn(0);  return Q;
      }
