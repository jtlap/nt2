...
//==============================================================================
#ifndef NT2_TRIG_DERIVATIVES_FUNCTIONS_D_SINE_HPP_INCLUDED
#define NT2_TRIG_DERIVATIVES_FUNCTIONS_D_SINE_HPP_INCLUDED
#include <nt2/include/functor.hpp>

namespace nt2 { namespace tag
  {
   /*!
     @brief d_sine generic tag

     Represents the d_sine function in generic contexts.

     @par Models:
        Hierarchy
   **/
    template <class T>
    struct d_sine_ : ext::elementwise_< d_sine_<T> >
    {
      typedef ext::elementwise_< d_sine_<T> > parent;
      template<class... Args>
      static BOOST_FORCEINLINE BOOST_AUTO_DECLTYPE dispatch(Args&&... args)
      BOOST_AUTO_DECLTYPE_HEADER( dispatching( ext::adl_helper(), *(d_sine_*)0, static_cast<Args&&>(args)... ) )
      {
        return dispatching( ext::adl_helper(), d_sine_(), static_cast<Args&&>(args)... );
      }
    };
  }
  /*!
    templated d_sine.
    \par
    d_sine is a templated version of cos
    ...
    use nt2::d_sine<big_> which is equivalent to nt2::sin
    \arg if you do not care for precision you can use
    nt2::d_sine<medium_> or nt2::d_sine<small_>
    ...
    d_sine<clipped_pio4_> have a perhaps still faster equivalent the non template
    nt2::fast_sin.
  ...
  **/
  NT2_FUNCTION_IMPLEMENTATION_TPL(tag::d_sine_<A0> , d_sine, (A1 const&), 2)
}
