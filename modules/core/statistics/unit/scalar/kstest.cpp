...
NT2_TEST_CASE_TPL ( normks,  NT2_REAL_TYPES)
{
  ...
  T mu = 0;
  T sig = 1;
  nt2::table<T> a = nt2::sort(normrnd(mu, sig, nt2::of_size(1, 10)), 2);
...
NT2_TEST_CASE_TPL ( cauks,  NT2_REAL_TYPES)
{
  ...
  T med = 0;
  T scal = 1;
  nt2::table<T> a = nt2::sort(normrnd(med, scal, nt2::of_size(1, 10)), 2);
...
NT2_TEST_CASE_TPL ( unifks,  NT2_REAL_TYPES)
{
  ...
  T a = -1;
  T b = 1;
  nt2::table<T> v = nt2::sort(normrnd(a, b, nt2::of_size(1, 10)), 2);
  unif_cdf<T>  cu(a, b);
  T d, p;
  nt2:: kstest(v, cu, d, p);
  //Kolmogorov smirnov at 5% must fail
