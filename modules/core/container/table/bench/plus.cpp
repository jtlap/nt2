...
template< typename Container
        ...
        >
NT2_EXPERIMENT(test_plus)
{
  public:
  test_plus ( std::size_t s0, std::size_t s1, bool status = false )
...
template<typename T> NT2_EXPERIMENT(test_plus< std::vector<T> >)
{
  public:
  test_plus ( std::size_t s0, std::size_t s1)
  ...
  virtual void run() const
  {
    for(std::size_t i=0; i<d0*d1; ++i) a2[i] = a0[i]+a1[i];
  }
...
NT2_RUN_EXPERIMENT_TPL( test_plus, (std::vector<T>) , (1<<N , 1<<N));       \
NT2_RUN_EXPERIMENT_TPL( test_plus, (nt2::table<T>)  , (1<<N , 1<<N,true));  \
NT2_RUN_EXPERIMENT_TPL( test_plus, (nt2::table<T>)  , (1<<N , 1<<N));       \
/**/
