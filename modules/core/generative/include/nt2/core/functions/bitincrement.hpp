...
namespace nt2
{
  ...
  /*!
    @brief Bitincrement generator

    Create an array full of the Bitincrement constant.

    @par Semantic:

    bitincrement() semantic depends of its parameters type and quantity:

    - The following code:
      @code
      double x = bitincrement();
      @endcode
      is equivalent to
      @code
      double x = Bitincrement<double>();
      @endcode

    - For any Integer @c n, the following code:
      @code
      auto x = bitincrement(n);
      @endcode
      generates an expression that evaluates as a @size2d{n,n} table filled with
      the <tt>Bitincrement<double>()</tt> value.

    - For any Integer @c sz1,...,szn , the following code:
      @code
      auto x = bitincrement(sz1,...,szn);
      @endcode
      generates an expression that evaluates as a @sizes{sz1,szn} table filled
      with the <tt>Bitincrement<double>()</tt> value.

    - For any Expression @c dims evaluating as a row vector of @c N elements,
      the following code:
      @code
      auto x = bitincrement(dims);
      @endcode
      generates an expression that evaluates as a @sizes{dims(1),dims(N)}
      table filled with the <tt>Bitincrement<double>()</tt> value.
    ...
    - For any Integer @c n and any type @c T, the following code:
      @code
      auto x = bitincrement(n, as_<T>());
      @endcode
      generates an expression that evaluates as a @size2d{n,n} table filled with
      the <tt>Bitincrement<T>()</tt> value.

    - For any Integer @c sz1,...,szN and any type @c T, the following code:
      @code
      auto x = bitincrement(sz1,...,szn, as_<T>());
      @endcode
      generates an expression that evaluates as a @sizes{sz1,szn} table filled
      with the <tt>Bitincrement<T>()</tt> value.

    - For any Expression @c dims evaluating as a row vector of @c N elements
      and any type @c T, the following code:
      @code
      auto x = bitincrement(dims, as_<T>());
      @endcode
      generates an expression that evaluates as a @sizes{dims(1),dims(N)}
      table filled with the <tt>Bitincrement<T>()</tt> value.
    ...
    @return An Expression evaluating as an array of a given type and dimensions
            filled with the @c Bitincrement constant.

    @sa Bitincrement
  **/
  template<typename... Args, typename ClassName>
  details::unspecified bitincrement(Args const&... dims, ClassName const& classname);

  /// @overload
  template<typename... Args> details::unspecified bitincrement(Args const&... dims);

  /// @overload
  template<typename ClassName> ClassName::type bitincrement(ClassName const& classname);
