...
//==============================================================================
// cover for functor almost_greater_or_equal in simd mode
...
NT2_TEST_CASE_TPL(almost_greater_or_equal_2,  NT2_SIMD_REAL_TYPES)
{
  ...
  typedef typename nt2::meta::call<nt2::tag::almost_greater_or_equal_(T, T, iT)>::type r_t;
  ...
  const iT min2_2 = args("min2_2", iT(0));
  const iT max2_2 = args("max2_2", iT(10));
  std::cout << "Argument samples #2 chosen in range: [" << min2_2 << ",  " << max2_2 << "]" << std::endl;
  NT2_CREATE_BUF(a2,iT, NR, min2_2, max2_2);

  std::vector<r_t> ref(NR);
  for(std::size_t i=0; i!=NR; ++i)
    ref[i] = nt2::almost_greater_or_equal(a0[i],a1[i],a2[i]);

  NT2_COVER_ULP_EQUAL(nt2::tag::almost_greater_or_equal_, ((vT, a0))((vT, a1))((ivT, a2)), ref, ulpd);
