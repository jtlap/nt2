...
//==============================================================================
// cover for functor almost_less_or_equal in simd mode
...
NT2_TEST_CASE_TPL(almost_less_or_equal_0,  NT2_SIMD_REAL_TYPES)
{
  ...
  const iT min2 = args("min2", iT(0));
  const iT max2 = args("max2", iT(10));
  std::cout << "Argument samples #2 chosen in range: [" << min2 << ",  " << max2 << "]" << std::endl;
  NT2_CREATE_BUF(a2,iT, NR, min2, max2);

  std::vector<r_t> ref(NR);
  for(std::size_t i=0; i!=NR; ++i)
    ref[i] = nt2::almost_less_or_equal(a0[i],a1[i],a2[i]);

  NT2_COVER_ULP_EQUAL(nt2::tag::almost_less_or_equal_, ((vT, a0))((vT, a1))((ivT, a2)), ref, ulpd);
