...
#ifdef __STDC__
static const double fd_atanhi[] = {
#else
static double fd_atanhi[] = {
#endif
...
#ifdef __STDC__
static const double fd_atanlo[] = {
#else
static double fd_atanlo[] = {
#endif
...
#endif
{
	...
	if(ix>=0x44100000) {	/* if |x| >= 2^66 */
	    if(ix>0x7ff00000||
		(ix==0x7ff00000&&(fd___LO(x)!=0)))
		return x+x;		/* NaN */
	    if(hx>0) return  fd_atanhi[3]+fd_atanlo[3];
	    else     return -fd_atanhi[3]-fd_atanlo[3];
	...
	if (id<0) return x - x*(s1+s2);
	    ...
	    z = fd_atanhi[id] - ((x*(s1+s2) - fd_atanlo[id]) - x);
	    return (hx<0)? -z:z;
	}
}
