...
class Mk_doc_skeleton_for_cephes(Create_py_doc_base) :
    def __init__(self, fct_name) :
        Create_py_doc_base.__init__(self, "cephes", fct_name)
    ...
    def get_included(self) :
        return "['#include <nt2/include/functions/%s.hpp>']"%self.get_fct_name()
    ...
    def get_property_call(self,arity)   :
        ...
        call = "nt2::cephes::%s(%s)"% (self.get_fct_name(),call_param)
...
if __name__ == "__main__" :
    ntp = Nt2_tb_props("ast")
    fcts = ntp.get_fcts_list()
##    fcts = ["sin"]
    for fct in  fcts :
        print(fct)
        mds = Mk_doc_skeleton_for_cephes(fct)
##//        print(mds)
