...
class Mk_doc_skeleton_for_libc(Create_py_doc_base) :
    def __init__(self, fct_name) :
        Create_py_doc_base.__init__(self, "libc", fct_name)
    ...
    def get_property_call(self,arity)   :
        ...
        call = "nt2::libc::%s(%s)"% (self.get_fct_name(),call_param)
...
if __name__ == "__main__" :
    ntp = Nt2_tb_props("libc")
    fcts = ntp.get_fcts_list()
##    fcts = ["abs"]
    for fct in  fcts :
        print(fct)
        mds = Mk_doc_skeleton_for_libc(fct)
##//        print(mds)
