...
def modify_defs(tb_name_list=None, fct_list0=None, style=None) :
...
##    PrettyPrinter().pprint(tb_name_list)
    
    for tb_name in tb_name_list :
        if tb_name != 'models' :
            ntbp = Nt2_tb_props(tb_name)
            if style is None or (style == ntbp.get_tb_style()):
                if fct_list0 is None :
                    fct_list =ntbp.get_fcts_list()
                else :
                    fct_list = fct_list0
                for fct in fct_list :
                    nud = Nt2_update_def(tb_name,fct)
                    nud.modify_def("vas-y")

       
if __name__ == "__main__" :
    modify_defs(tb_name_list="libc",style='usr')
