...
$verif_test$
    ...
    Doc_specific_values_dict = """            'real_'       : {
    ...
    Doc_verif_test = (
    """\
        'property_call' : { 'default' :  ['nt2::$name$($params$)'], },
        'property_value' :{ 'default' :  ['::cephes_$name$l($params$)'], },
    ...
    def mk_fct_doc_tpl(self,
                       arity = '1') :
        n = 4
        doc_tpl = Nt2_fct_doc.Doc_tpl
        doc_functor = (' '*n)+Nt2_fct_doc.Doc_functor.replace('\n', '\n'+' '*n)
        doc_global_header = (' '*n)+Nt2_fct_doc.Doc_global_header.replace('\n', '\n'+' '*n)
        doc_specific_values = Nt2_fct_doc.Doc_specific_values_dict
##            doc_specific_values = self.__specif_vals(Nt2_fct_internals.get_returns(self.get_fct_text()).keys(),n,arity)
        doc_ranges = (' '*n)+Nt2_fct_doc.Doc_ranges.replace('\n', '\n'+' '*n)
        
        dvf = Nt2_fct_doc.Doc_verif_test.replace('$name$', self.get_fct_name())
        dvf = dvf.replace('$params$', 'a0')
        doc_verif_test = (' '*n)+dvf.replace('\n', '\n'+' '*n)
        s = Nt2_fct_doc.Doc_tpl
        ...
        s = re.sub('\$ranges\$', doc_ranges,s)
        return s

    def mk_doc_from_existing_fct(self,mk_doc_tpl=None) :
#        if doc_tpl is None : doc_tpl = self.mk_fct_doc_tpl()
        docs= Nt2_fct_doc.Doc_beg
        for a in self.get_fct_arities():
            adoc = self.__populate_doc(mk_doc=None,arity=a)
            docs += adoc
        docs += Nt2_fct_doc.Doc_end
        return docs

    def create_fct_pydoc_file(self, force=True,error='warn') :
...
##            doc = mk_doc(arity=arity)
        ...
        doc = re.sub('\$types\$'     , "'real_'",doc)   ##','.join(self.get_fct_types()),doc)
