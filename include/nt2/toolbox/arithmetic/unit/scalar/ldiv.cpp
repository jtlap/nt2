...
NT2_TEST_CASE_TPL ( ldiv_real__2,  NT2_REAL_TYPES)
{
  using nt2::ldiv;
  using nt2::tag::ldiv_;
  ...
  NT2_TEST_ULP_EQUAL(  ldiv(nt2::Inf<T>(), nt2::Inf<T>()), nt2::Nan<T>(), 0);
  NT2_TEST_ULP_EQUAL(  ldiv(nt2::Minf<T>(), nt2::Minf<T>()), nt2::Nan<T>(), 0);
  NT2_TEST_ULP_EQUAL(  ldiv(nt2::Mone<T>(), nt2::Mone<T>()), nt2::One<T>(), 0);
  NT2_TEST_ULP_EQUAL(  ldiv(nt2::Nan<T>(), nt2::Nan<T>()), nt2::Nan<T>(), 0);
  NT2_TEST_ULP_EQUAL(  ldiv(nt2::One<T>(), nt2::One<T>()), nt2::One<T>(), 0);
  NT2_TEST_ULP_EQUAL(  ldiv(nt2::Zero<T>(), nt2::Zero<T>()), nt2::Nan<T>(), 0);
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for params "
                  ...
                  << ", a1 = "<< u_t(a1 = tab_a1[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( ldiv(a0,a1),a1/a0,0);
...
NT2_TEST_CASE_TPL ( ldiv_unsigned_int__2,  NT2_UNSIGNED_TYPES)
{
  using nt2::ldiv;
  using nt2::tag::ldiv_;
  typedef typename nt2::meta::as_integer<T>::type iT;
  ...
  static const uint32_t NR = NT2_NB_RANDOM_TEST;
  {
    NT2_CREATE_SCALAR_BUFFER(a0,T, NR, 1, 100);
...
NT2_TEST_CASE_TPL ( ldiv_signed_int__2,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::ldiv;
  using nt2::tag::ldiv_;
  typedef typename nt2::meta::as_integer<T>::type iT;
