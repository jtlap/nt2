...
NT2_TEST_CASE_TPL ( rem_real__2,  NT2_REAL_TYPES)
{
  using nt2::rem;
  using nt2::tag::rem_;
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for params "
                  ...
                  << ", a1 = "<< u_t(a1 = tab_a1[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::rem(a0,a1),a1 ? a0-a1*nt2::idivfix(a0,a1) : a0,0);
     }
   }
} // end of test for real_

NT2_TEST_CASE_TPL ( rem_unsigned_int__2,  NT2_UNSIGNED_TYPES)
{
  using nt2::rem;
  using nt2::tag::rem_;
  typedef typename nt2::meta::call<rem_(T,T)>::type r_t;
...
NT2_TEST_CASE_TPL ( rem_signed_int__2,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::rem;
  using nt2::tag::rem_;
  typedef typename nt2::meta::call<rem_(T,T)>::type r_t;
