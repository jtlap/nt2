...
NT2_TEST_CASE_TPL ( fma_real_,  NT2_REAL_TYPES)
{
  using nt2::fma;
  ...
  for (int j =0; j < N; ++j )
    {
      std::cout << "for params "
                ...
                << ", a2 = "<< u_t(a2 = tab_a2[j])
                << std::endl;
      NT2_TEST_ULP_EQUAL( nt2::fma(a0,a1,a2), a0*a1+a2,0);
   }
} // end of test for real_

NT2_TEST_CASE_TPL ( fma_unsigned_int_,  NT2_UNSIGNED_TYPES)
{
  using nt2::fma;
  using nt2::functors::fma_;
  typedef typename nt2::meta::call<fma_(T, T, T)>::type r_t;
...
NT2_TEST_CASE_TPL ( fma_signed_int_,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::fma;
  using nt2::functors::fma_;
  typedef typename nt2::meta::call<fma_(T, T, T)>::type r_t;
