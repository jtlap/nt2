...
NT2_TEST_CASE_TPL(rdivide, NT2_SIMD_TYPES )
{
 using nt2::rdivide;
 ...
 typedef typename nt2::meta::call<rdivide_(n_t, n_t)>::type call_type;
 ...
 for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = i+1; // good value here for rdivide
 }
   ...
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], rdivide(a0[j], a1[j]) );
     }
 }
