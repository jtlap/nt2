...
NT2_TEST_CASE_TPL(dist, (double)(float) )
{
 ...
 typedef typename nt2::meta::call<dist_(n_t, n_t)>::type call_type;
 ...
 for(std::size_t i=0;i<3*cardinal_of<n_t>::value;++i){
   data[i] = i-T(cardinal_of<n_t>::value)/2; // good value here for dist
 }
   ...
   {
     NT2_TEST_EQUAL( v[j], dist(a0[j], a1[j]) );
   }
 }
