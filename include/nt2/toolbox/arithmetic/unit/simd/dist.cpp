...
NT2_TEST_CASE_TPL ( dist_real__2,  NT2_REAL_TYPES)
{
  ...
  {
    NT2_CREATE_SIMD_BUFFER(a0,T, NR, nt2::Valmin<T>()/2, nt2::Valmax<T>()/2);
    NT2_CREATE_SIMD_BUFFER(a1,T, NR, nt2::Valmin<T>(), nt2::Valmax<T>());
    ...
    for(int j = 0; j < NR/cardinal_of<n_t>::value; j++)
      {
        ...
        for(int i = 0; i< cardinal_of<n_t>::value; i++)
        {
          ...
          NT2_TEST_ULP_EQUAL( v[i],nt2::dist(tab_a0[k],tab_a1[k]),2.5);
