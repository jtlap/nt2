...
NT2_TEST_CASE_TPL(mod, NT2_SIMD_TYPES )
{
 using nt2::mod;
 using nt2::tag::mod_;    
 using nt2::load;  
 ...
 typedef typename nt2::meta::call<mod_(n_t, n_t)>::type call_type;
 ...
 for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = (5*i+1)/3; // good value here for mod
 }
   ...
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], mod(a0[j], a1[j]) );
     }
 }
