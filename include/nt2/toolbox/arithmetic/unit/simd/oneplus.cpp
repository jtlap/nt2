...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(oneplus, NT2_SIMD_TYPES )
{
 using nt2::oneplus;
 ...
 typedef typename nt2::meta::call<oneplus_(n_t)>::type call_type;

 NT2_TEST( (boost::is_same<call_type, n_t>::value) );  
 NT2_ALIGNED_TYPE(T) data[1*cardinal_of<n_t>::value]; 
 for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
   data[i] = i-T(cardinal_of<n_t>::value)/2; // good value here for oneplus
 }
   n_t a0 = load<n_t>(&data[0],0); 
   n_t v  = oneplus(a0);
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
   {
     NT2_TEST_EQUAL( v[j], oneplus(a0[j]) );
   }
 }
