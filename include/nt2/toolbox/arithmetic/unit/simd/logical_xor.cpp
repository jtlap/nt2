...
NT2_TEST_CASE_TPL(logical_xor, NT2_SIMD_TYPES )
{
 ...
 typedef typename nt2::meta::call<logical_xor_(n_t, n_t)>::type call_type;
  ...
  for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){    
    data[i] = (5*i+1)/3; // good value here for logical_xor
  }
  n_t a0 = load<n_t>(&data[0],0);   
  n_t a1 = load<n_t>(&data[0],1);
  n_t v  = logical_xor(a0, a1);
  for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
    {
      NT2_TEST_EQUAL( nt2::boolean(v[j]), logical_xor(a0[j], a1[j]) );
    }
 }
