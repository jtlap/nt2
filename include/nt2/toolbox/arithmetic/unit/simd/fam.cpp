...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(fam,  (nt2::int16_t)
		  (nt2::int8_t)  )
{
 using nt2::fam;
 ...
 typedef typename nt2::meta::call<fam_(n_t, n_t, n_t)>::type call_type;
 ...
 for(std::size_t i=0;i<3*cardinal_of<n_t>::value;++i){
   data[i] = (i-T(cardinal_of<n_t>::value)/2)%8; // good value here for fam
 }
...
NT2_TEST_CASE_TPL(big_fam,  (nt2::int64_t)(double)
	          (nt2::int32_t)(float)  )
{
 using nt2::fam;
 using nt2::functors::fam_;    
 ...
 NT2_ALIGNED_TYPE(T) data[3*cardinal_of<n_t>::value]; 
 for(std::size_t i=0;i<3*cardinal_of<n_t>::value;++i){
   data[i] = (i-T(cardinal_of<n_t>::value)/2); // good value here for fam
 }
   n_t a0 = load<n_t>(&data[0],0); 
   n_t a1 = load<n_t>(&data[0],1); 
   n_t a2 = load<n_t>(&data[0],2); 
	          ...
	          (nt2::uint64_t)  )
{
 ...
 NT2_ALIGNED_TYPE(T) data[3*cardinal_of<n_t>::value]; 
 for(std::size_t i=0;i<3*cardinal_of<n_t>::value;++i){
   data[i] = i%8; // good value here for fam
 }
   n_t a0 = load<n_t>(&data[0],0); 
   n_t a1 = load<n_t>(&data[0],1); 
   n_t a2 = load<n_t>(&data[0],2); 
