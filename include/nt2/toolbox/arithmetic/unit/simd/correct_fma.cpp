...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(correct_fma, (double)(float) )
{
 ...
 typedef typename nt2::meta::call<correct_fma_(n_t, n_t, n_t)>::type call_type;
 ...
 for(std::size_t i=0;i<3*cardinal_of<n_t>::value;++i){
   data[i] = i-T(cardinal_of<n_t>::value)/2; // good value here for correct_fma
 }
   n_t a0 = load<n_t>(&data[0],0); 
   n_t a1 = load<n_t>(&data[0],1); 
   n_t a2 = load<n_t>(&data[0],2); 
   n_t v  = correct_fma(a0, a1, a2);
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
   {
     NT2_TEST_EQUAL( v[j], correct_fma(a0[j], a1[j], a2[j]) );
   }
 }
