...
NT2_TEST_CASE_TPL(bitwise_notor, NT2_SIMD_TYPES )
{
 ...
 typedef typename nt2::meta::call<bitwise_notor_(n_t, n_t)>::type call_type;
  ...
  for(int i=0;i<2*cardinal_of<n_t>::value;++i){    
    data[i] = nt2::random(-100.0, 100.0); // good value here for bitwise_notor
  }
