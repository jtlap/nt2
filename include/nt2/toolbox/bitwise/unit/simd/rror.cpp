...
NT2_TEST_CASE_TPL ( rror_integer__2,  NT2_INTEGRAL_TYPES)
{
  ...
  typedef typename nt2::meta::call<rror_(vT,vT)>::type r_t;

  // random verifications
  static const uint32_t NR = NT2_NB_RANDOM_TEST;
  {
    NT2_CREATE_SIMD_BUFFER(a0,T, NR, T(-10000), T(10000));
    NT2_CREATE_SIMD_BUFFER(a1,T, NR, T(0), sizeof(T)*8-1);
    ...
    for(int j = 0; j < NR/cardinal_of<n_t>::value; j++)
      {
        ...
        for(int i = 0; i< cardinal_of<n_t>::value; i++)
        {
          ...
          NT2_TEST_ULP_EQUAL( v[i],nt2::rror(tab_a0[k],tab_a1[k]),1.5);
