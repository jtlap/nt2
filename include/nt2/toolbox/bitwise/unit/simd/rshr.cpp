...
NT2_TEST_CASE_TPL(rshr, NT2_SIMD_INTEGRAL_TYPES )
{
 using nt2::rshr;
 using nt2::tag::rshr_;    
 using nt2::load;  
 ...
 typedef typename nt2::meta::call<rshr_(n_t, n_t)>::type call_type;
  ...
  for(int i=0;i<cardinal_of<n_t>::value;++i){    
    data[i] = nt2::random(-100.0, 100.0); // good value here for rshr
   ...
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], rshr(a0[j], a1[j]) );
     }
 }
