...
NT2_TEST_CASE_TPL(bitwise_select, NT2_SIMD_TYPES )
{
 ...
 typedef typename nt2::meta::call<bitwise_select_(n_t, n_t, n_t)>::type call_type;

  NT2_TEST( (boost::is_same<call_type, n_t>::value) );  
  NT2_ALIGNED_TYPE(T) data[2*cardinal_of<n_t>::value];
  NT2_ALIGNED_TYPE(T) dat0[  cardinal_of<in_t>::value];  
 for(int i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = nt2::random(0.0, 100.0); // good value here for bitwise_select
 }
 for(int i=0;i<cardinal_of<in_t>::value;++i){    
   dat0[i] = nt2::random(-1, 0); // good value here for bitwise_select
 }
   n_t a0 = load<n_t>(&dat0[0],0);   
   n_t a1 = load<n_t>(&data[0],0);
   n_t a2 = load<n_t>(&data[0],1);
   n_t v  = bitwise_select(a0, a1, a2); 
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], bitwise_select(a0[j], a1[j], a2[j]) );
     }
 }
