...
NT2_TEST_CASE_TPL(bitwise_notand, NT2_SIMD_TYPES )
{
 ...
 typedef typename nt2::meta::call<bitwise_notand_(n_t, n_t)>::type call_type;
 ...
 for(int i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = nt2::random(-100.0, 100.0); // good value here for bitwise_notand
 }
   n_t a0 = load<n_t>(&data[0],0);   
   n_t a1 = load<n_t>(&data[0],1);
   n_t v  = bitwise_notand(a0, a1);
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], bitwise_notand(a0[j], a1[j]) );
     }
 }
