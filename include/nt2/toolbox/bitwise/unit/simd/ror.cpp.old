...
NT2_TEST_CASE_TPL(ror, NT2_SIMD_INTEGRAL_TYPES )
{
 using nt2::ror;
 ...
 typedef typename nt2::meta::call<ror_(n_t, n_t)>::type call_type;
  ...
  for(int i=0;i<cardinal_of<n_t>::value;++i){    
    data[i] = nt2::random(-100.0, 100.0); // good value here for ror
   ...
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], ror(a0[j], a1[j]) );
     }
 }
