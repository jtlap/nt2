...
NT2_TEST_CASE_TPL(rol, NT2_SIMD_INTEGRAL_TYPES )
{
 using nt2::rol;
 using nt2::tag::rol_;    
 using nt2::load;  
 ...
 typedef typename nt2::meta::call<rol_(n_t, n_t)>::type call_type;
  ...
  for(int i=0;i<cardinal_of<n_t>::value;++i){    
    data[i] = nt2::random(-100.0, 100.0); // good value here for rol
   ...
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], rol(a0[j], a1[j]) );
     }
 }
