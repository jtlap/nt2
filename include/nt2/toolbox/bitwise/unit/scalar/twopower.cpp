...
NT2_TEST_CASE_TPL ( twopower_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  ...
  NT2_TEST_ULP_EQUAL(  twopower(nt2::Zero<T>()), nt2::One<r_t>(), 0.5);
  // random verifications
  static const uint32_t NR = NT2_NB_RANDOM_TEST;
  {
    NT2_CREATE_SCALAR_BUFFER(a0,T, NR, 0, sizeof(T)*8-1);
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::twopower(a0),nt2::shli(T(1),a0),0);
