...
NT2_TEST_CASE_TPL ( shri_unsigned_int__2,  NT2_UNSIGNED_TYPES)
{
  using nt2::shri;
  using nt2::tag::shri_;
  typedef typename nt2::meta::call<shri_(T,T)>::type r_t;
  ...
  NT2_TEST_ULP_EQUAL(  shri(nt2::Mone<T>(),(sizeof(T)*8-1)), nt2::One<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  shri(nt2::Mone<T>(),(sizeof(T)*8-2)), nt2::Three<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  shri(nt2::One<T>(),1), nt2::Zero<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  shri(nt2::Two<T>(),1), nt2::One<T>(), 0.5);
  NT2_TEST_ULP_EQUAL(  shri(nt2::Zero<T>(),1), nt2::Zero<r_t>(), 0.5);
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for params "
                  ...
                  << ", a1 = "<< u_t(a1 = tab_a1[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::shri(a0,a1),a0/nt2::twopower(a1),0);
