 ...
 *
 * Accuracy for large arguments is dominated by error in cephes_powl().
 *
 */
/*							cephes_lgaml()
 *
...
#else
long double cephes_fabsl(), cephes_lgaml(), cephes_logl(), cephes_expl(), cephes_gammal(), cephes_sinl();
long double cephes_floorl(), cephes_powl(), cephes_polevll(), cephes_p1evll(), cephes_isnanl(), cephes_isfinitel();
...
if( x > MAXSTIR )
	...
	v = cephes_powl( x, 0.5L * x - 0.25L );
	y = v * (v / y);
	}
else
	{
	y = cephes_powl( x, x - 0.5L ) / y;
	}
y = SQTPI * y * w;
return( y );
}
...
gamnan:
		...
		z = q * cephes_sinl( PIL * z );
		z = cephes_fabsl(z) * stirf(q);
		if( z <= PIL/MAXNUML )
			{
goverf:
#ifdef INFINITIES
...
#endif
#ifdef INFINITIES
if( !cephes_isfinitel(x) )
	return(INFINITYL);
#endif
if( x < -34.0L )
	{
	q = -x;
	w = cephes_lgaml(q); /* note this modifies sgngam! */
	p = cephes_floorl(q);
	if( p == q )
		{
#ifdef INFINITIES
		cephes_mtherr( "lgaml", SING );
		return (INFINITYL);
#else
		goto loverf;
#endif
	...
	if( z == 0.0L )
		goto loverf;
/*	z = LOGPI - cephes_logl( z ) - w; */
	z = cephes_logl( PIL/z ) - w;
	return( z );
	}
...
loverf:
	...
	return( sgngaml * INFINITYL );
#else
	cephes_mtherr( "lgaml", OVERFLOW );
	return( sgngaml * MAXNUML );
#endif
	}

q = ( x - 0.5L ) * cephes_logl(x) - x + LS2PI;
if( x > 1.0e10L )
	return(q);
p = 1.0L/(x*x);
