...
#else
long double cephes_polevll(), cephes_p1evll(), cephes_expl(), cephes_logl(), cephes_erfl(), cephes_erfcl(), cephes_fabsl();
long double cephes_expx2l(), cephes_sqrtl();
...
/* Compute z = cephes_expl(z).  */
z = cephes_expx2l(a, -1);
y = 1.0L/x;

if( x < 8.0L )
	{
	p = cephes_polevll( y, P, 9 );
	q = cephes_p1evll( y, Q, 10 );
	}
else
	{
	q = y * y;
	p = y * cephes_polevll( q, R, 4 );
	q = cephes_p1evll( q, S, 5 );
	}
y = (z * p)/q;
...
if( cephes_fabsl(x) > 1.0L )
	return( 1.0L - cephes_erfcl(x) );

z = x * x;
y = x * cephes_polevll( z, T, 6 ) / cephes_p1evll( z, U, 6 );
return( y );
}
