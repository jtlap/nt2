 ...
 * SYNOPSIS:
 *
 * long double a, b, x, y, cephes_incbil();
 *
 * x = cephes_incbil( a, b, y );
 *
 *
 *
 * DESCRIPTION:
...
#else
long double cephes_incbetl(), cephes_expl(), cephes_fabsl(), cephes_logl(), cephes_sqrtl(), cephes_lgaml();
long double cephes_ndtril();
#endif

long double cephes_incbil( aa, bb, yy0 )
...
x = a/( a + b * cephes_expl(d) );
y = cephes_incbetl( a, b, x );
yp = (y - y0)/y0;
if( cephes_fabsl(yp) < 0.2 )
	goto newt;
...
if( x <= 0.0L )
	{
under:
	cephes_mtherr( "incbil", UNDERFLOW );
	x = 0.0L;
	goto done;
	}
...
if( nflg )
	goto done;
nflg = 1;
lgm = cephes_lgaml(a+b) - cephes_lgaml(a) - cephes_lgaml(b);
...
for( i=0; i<15; i++ )
	...
	d = (a - 1.0L) * cephes_logl(x) + (b - 1.0L) * cephes_logl(1.0L - x) + lgm;
	if( d < MINLOGL )
		goto done;
	if( d > MAXLOGL )
		break;
	...
	x = xt;
	if( cephes_fabsl(d/x) < (128.0L * MACHEPL) )
		goto done;
	}
/* Did not converge.  */
