...
#else
long double cephes_y0l(), cephes_y1l();
#endif

long double cephes_ynl( n, x )
int n;
long double x;
{
...
if( n == 0 )
	return( sign * cephes_y0l(x) );
if( n == 1 )
	return( sign * cephes_y1l(x) );

/* test for overflow */
if( x <= 0.0L )
	{
	cephes_mtherr( "ynl", SING );
	return( -MAXNUML );
	}
