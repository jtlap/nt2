 ...
 * SYNOPSIS:
 *
 * long double a, b, x, y, cephes_incbetl();
 *
 * y = cephes_incbetl( a, b, x );
 *
 *
 * DESCRIPTION:
 *
...
#else
long double cephes_gammal(), cephes_lgaml(), cephes_expl(), cephes_logl(), cephes_fabsl(), cephes_powl();
static long double incbcfl(), incbdl(), pseriesl();
#endif

long double cephes_incbetl( aa, bb, xx )
...
y = a * cephes_logl(x);
t = b * cephes_logl(xc);
if( (a+b) < MAXGAML && cephes_fabsl(y) < MAXLOGL && cephes_fabsl(t) < MAXLOGL )
	{
	t = cephes_powl(xc,b);
	t *= cephes_powl(x,a);
	t /= a;
	t *= w;
	t *= cephes_gammal(a+b) / (cephes_gammal(a) * cephes_gammal(b));
	goto done;
	}
else
	{
	/* Resort to logarithms.  */
	y += t + cephes_lgaml(a+b) - cephes_lgaml(a) - cephes_lgaml(b);
	y += cephes_logl(w/a);
	if( y < MINLOGL )
		t = 0.0L;
	else
...
do
	...
	if( (cephes_fabsl(qk) + cephes_fabsl(pk)) > big )
		{
		pkm2 *= biginv;
		pkm1 *= biginv;
		qkm2 *= biginv;
	...
	if( (cephes_fabsl(qk) < biginv) || (cephes_fabsl(pk) < biginv) )
		{
		pkm2 *= big;
		pkm1 *= big;
		qkm2 *= big;
...
u = a * cephes_logl(x);
if( (a+b) < MAXGAML && cephes_fabsl(u) < MAXLOGL )
	{
	t = cephes_gammal(a+b)/(cephes_gammal(a)*cephes_gammal(b));
	s = s * t * cephes_powl(x,a);
	}
else
	{
	t = cephes_lgaml(a+b) - cephes_lgaml(a) - cephes_lgaml(b) + u + cephes_logl(s);
	if( t < MINLOGL )
		s = 0.0L;
	else
	s = cephes_expl(t);
