...
while( (*p  & 0x8000) == 0)
	...
	if( sc > (NBITS+16) )
		{
		cephes_mtherr( "enormlz", UNDERFLOW );
		return( sc );
		}
	}
return( sc );
...
while( *p != 0 )
	...
	if( sc < -NBITS )
		{
		cephes_mtherr( "enormlz", OVERFLOW );
		return( sc );
		}
	}
return( sc );
