...
#ifdef ANSIC
float cephes_floorf(float), cephes_j0f(float), cephes_j1f(float);
...
float cephes_sqrtf(float), cephes_sinf(float), cephes_cosf(float);
float cephes_lgamf(float), cephes_expf(float), cephes_logf(float), cephes_powf(float, float);
float cephes_gammaf(float), cephes_cbrtf(float), cephes_acosf(float);
int cephes_airyf(float, float *, float *, float *, float *);
float cephes_polevlf(float, float *, int);
#else
float cephes_floorf(), cephes_j0f(), cephes_j1f();
float cephes_sqrtf(), cephes_sinf(), cephes_cosf();
float cephes_lgamf(), cephes_expf(), cephes_logf(), cephes_powf(), cephes_gammaf();
float cephes_cbrtf(), cephes_polevlf(), cephes_acosf();
void cephes_airyf();
...
#ifdef ANSIC
float cephes_jvf( float nn, float xx )
#else
float cephes_jvf( nn, xx )
double nn, xx;
#endif
{
...
if( y == an )
	{
	nint = 1;
	i = an - 16384.0 * cephes_floorf( an/16384.0 );
	if( n < 0.0 )
		{
		if( i & 1 )
			sign = -sign;
...
t = 3.6 * cephes_sqrtf(an);
if( y < t )
	return( sign * jvsf(n,x) );

/* x large compared to n */
k = 3.6 * cephes_sqrtf(y);
if( (an < k) && (y > 6.0) )
...
if( x < 0.0 )
	{
	y = y * cephes_powf( 0.5 * x, n ) / cephes_gammaf( n + 1.0 );
	}
else
	{
	t = n * cephes_logf(0.5*x) - cephes_lgamf(n + 1.0);
	if( t < -MAXLOGF )
		{
		return( 0.0 );
		}
...
hank1:
u = x - (0.5*n + 0.25) * PIF;
t = cephes_sqrtf( 2.0/(PIF*x) ) * ( pp * cephes_cosf(u) - qq * cephes_sinf(u) );
return( t );
}

...
if( zz > 0.0 )
	{
	sz = cephes_sqrtf( zz );
	t = 1.5 * (cephes_logf( (1.0+sz)/z ) - sz );	/* zeta ** 3/2		*/
...
#endif
cephes_airyf( t, &ai, &aip, &bi, &bip );

/* polynomials in expansion */
u[0] = 1.0;
zzi = 1.0/zz;
u[1] = cephes_polevlf( zzi, P1, 1 )/sz;
u[2] = cephes_polevlf( zzi, P2, 2 )/zz;
u[3] = cephes_polevlf( zzi, P3, 3 )/(sz*zz);
pp = zz*zz;
u[4] = cephes_polevlf( zzi, P4, 4 )/pp;
u[5] = cephes_polevlf( zzi, P5, 5 )/(pp*sz);
pp *= zz;
u[6] = cephes_polevlf( zzi, P6, 6 )/pp;
u[7] = cephes_polevlf( zzi, P7, 7 )/(pp*sz);
...
t *= ai*pp/cephes_cbrtf(n)  +  aip*qq/(n23*n);
return(t);
}

...
cbn = cephes_cbrtf(n);
z = (x - n)/cbn;
cbtwo = cephes_cbrtf( 2.0 );

/* Airy function */
zz = -cbtwo * z;
cephes_airyf( zz, &ai, &aip, &bi, &bip );
...
F[0] = 1.0;
F[1] = -z/5.0;
F[2] = cephes_polevlf( z3, PF2, 1 ) * zz;
F[3] = cephes_polevlf( z3, PF3, 2 );
F[4] = cephes_polevlf( z3, PF4, 3 ) * z;
G[0] = 0.3 * zz;
G[1] = cephes_polevlf( z3, PG1, 1 );
G[2] = cephes_polevlf( z3, PG2, 2 ) * z;
...
fk = cbtwo * ai * pp/cbn  +  cephes_cbrtf(4.0) * aip * qq/n;
return(fk);
}
