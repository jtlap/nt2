...
#ifdef ANSIC
float cephes_polevlf(float, float *, int), cephes_floorf(float), cephes_ldexpf(float, int);

float cephes_exp2f( float xx )
#else
float cephes_polevlf(), cephes_floorf(), cephes_ldexpf();

float cephes_exp2f(xx)
double xx;
#endif
{
float x, px;
