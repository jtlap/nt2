...
float cephes_expf(float), cephes_i1f(float), cephes_logf(float), cephes_sqrtf(float);
#else
float cephes_chbevlf(), cephes_expf(), cephes_i1f(), cephes_logf(), cephes_sqrtf();
#endif

#ifdef ANSIC
float cephes_k1f(float xx)
#else
float cephes_k1f(xx)
double xx;
#endif
...
if( x <= 2.0f )
	{
	y = x * x - 2.0f;
	y =  cephes_logf( 0.5f * x ) * cephes_i1f(x)  +  cephes_chbevlf( y, A, 7 ) / x;
	return( y );
	}

return(  cephes_expf(-x) * cephes_chbevlf( 8.0f/x - 2.0f, B, 10 ) / cephes_sqrtf(x) );
...
#ifdef ANSIC
float cephes_k1ef( float xx )
#else
float cephes_k1ef( xx )
double xx;
#endif
{
float x, y;
...
if( x <= 2.0f )
	...
	y = x * x - 2.0f;
	y =  cephes_logf( 0.5f * x ) * cephes_i1f(x)  +  cephes_chbevlf( y, A, 7 ) / x;
	return( y * cephes_expf(x) );
	}

return(  cephes_chbevlf( 8.0f/x - 2.0f, B, 10 ) / cephes_sqrtf(x) );
