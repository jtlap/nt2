...
float cephes_expf(float), cephes_i0f(float), cephes_logf(float), cephes_sqrtf(float);
#else
float cephes_chbevlf(), cephes_expf(), cephes_i0f(), cephes_logf(), cephes_sqrtf();
#endif


#ifdef ANSIC
float cephes_k0f( float xx )
#else
float cephes_k0f(xx)
double xx;
#endif
...
if( x <= 2.0f )
	{
	y = x * x - 2.0f;
	y = cephes_chbevlf( y, A, 7 ) - cephes_logf( 0.5f * x ) * cephes_i0f(x);
	return( y );
	}
z = 8.0f/x - 2.0f;
y = cephes_expf(-x) * cephes_chbevlf( z, B, 10 ) / cephes_sqrtf(x);
return(y);
}
...
#ifdef ANSIC
float cephes_k0ef( float xx )
#else
float cephes_k0ef( xx )
double xx;
#endif
{
float x, y;
...
y = cephes_chbevlf( 8.0f/x - 2.0f, B, 10 ) / cephes_sqrtf(x);
return(y);
}
