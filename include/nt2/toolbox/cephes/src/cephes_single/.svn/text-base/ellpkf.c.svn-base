...
float cephes_ellpkf(float xx)
#else
float cephes_polevlf(), cephes_p1evlf(), cephes_logf();
float cephes_ellpkf(xx)
double xx;
#endif
{
float x;
...
if( x > MACHEPF )
	{
	return( cephes_polevlf(x,P,10) - cephes_logf(x) * cephes_polevlf(x,Q,10) );
	}
else
	{
	if( x == 0.0 )
