...
float cephes_logf(float), cephes_sinf(float), cephes_cosf(float), cephes_sqrtf(float);

float cephes_j0f( float xx )
#else
float cephes_polevlf(), cephes_logf(), cephes_sinf(), cephes_cosf(), cephes_sqrtf();
...
p = w * cephes_polevlf( q, MO, 7);
w = q*q;
xn = q * cephes_polevlf( w, PH, 7) - PIO4F;
p = p * cephes_cosf(xn + x);
return(p);
}

/*							y0() 2	*/
...
if( x <= 2.0f )
	...
	if( x <= 0.0f )
		{
		cephes_mtherr( "y0f", DOMAIN );
		return( -MAXNUMF );
		}
	z = x * x;
/*	w = (z-YZ1)*(z-YZ2)*(z-YZ3) * cephes_polevlf( z, YP, 4);*/
	w = (z-YZ1) * cephes_polevlf( z, YP, 4);
	w += TWOOPI * cephes_logf(x) * cephes_j0f(x);
	return( w );
	}
