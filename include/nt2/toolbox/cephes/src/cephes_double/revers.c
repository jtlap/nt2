 ...
 * revers( y, x, n );
 *
 *  Note, cephes_polini() initializes the polynomial arithmetic subroutines;
 *  see polyn.c.
 *
 *
 * DESCRIPTION:
...
extern int MAXPOL; /* initialized by cephes_polini() */

#ifdef ANSIPROT
/* See polyn.c.  */
void cephes_polmov ( double *, int, double * );
void cephes_polclr ( double *, int );
void cephes_poladd ( double *, int, double *, int, double * );
void cephes_polmul ( double *, int, double *, int, double * );
void * malloc ( long );
void free ( void * );
#else
void cephes_polmov(), cephes_polclr(), cephes_poladd(), cephes_polmul();
void * malloc();
void free ();
#endif
...
cephes_polmov( y, n, yn );
cephes_polclr( ysum, n );
x[0] = 0.0;
x[1] = 1.0/y[1];
for( j=2; j<=n; j++ )
	{
/* A_(j-1) times the expansion of y^(j-1)  */
	cephes_polmul( &x[j-1], 0, yn, n, yp );
/* The expansion of the sum of A_k y^k up to k=j-1 */
	cephes_poladd( yp, n, ysum, n, ysum );
/* The expansion of y^j */
	cephes_polmul( yn, n, y, n, yn );
...
/*
	if( i & 1 )
		{
		y[i] = odd/cephes_fac(i);
		odd = -odd;
		}
	else
		y[i] = 0.0;
*/
	y[i] = 1.0/cephes_fac(i);
	}
revers( y, x, N-1 );
for( i=0; i<N; i++ )
	printf( "%2d %.10e %.10e\n", i, x[i], y[i] );
