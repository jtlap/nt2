 ...
 *
 *	cephes_sqrt(z) * P(z)/Q(z)
 *
 * where z = x-1, is used.  Otherwise,
 *
 * cephes_acosh(x)  =  cephes_log( x + cephes_sqrt( (x-1)(x+1) ).
 *
 *
 *
 * ACCURACY:
...
/* cephes_acosh(z) = cephes_sqrt(x) * R(x), z = x + 1, interval 0 < x < 0.5 */
...
#else
double cephes_log(), cephes_sqrt(), cephes_polevl(), cephes_p1evl();
#endif
