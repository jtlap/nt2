 ...
 *                             -
 *                   1        | |          2
 *    cephes_ndtr(x)  = ---------    |    cephes_exp( - t /2 ) dt
 *               cephes_sqrt(2pi)  | |
 *                           -
 *                          -inf.
 *
 ...
 *             =  cephes_erfc(z) / 2
 *
 * where z = x/cephes_sqrt(2). Computation is via the functions
 * erf and erfc with care to avoid error amplification in computing cephes_exp(-x^2).
 *
 *
 * ACCURACY:
 *
 ...
 *
 * For 0 <= |x| < 1, cephes_erf(x) = x * P4(x**2)/Q5(x**2); otherwise
 * cephes_erf(x) = 1 - cephes_erfc(x).
 *
 *
 *
 * ACCURACY:
 ...
 *                             -
 *                  2         | |          2
 *   cephes_erfc(x)  =  --------     |    cephes_exp( - t  ) dt
 *               cephes_sqrt(pi)   | |
 *                           -
 *                            x
 *
 *
 * For small x, cephes_erfc(x) = 1 - cephes_erf(x); otherwise rational
 * approximations are computed.
 *
 * A special function expx2.c is used to suppress error amplification
 * in computing cephes_exp(-x^2).
 *
 *
 * ACCURACY:
 *
...
/* Define this macro to suppress error propagation in cephes_exp(x^2)
