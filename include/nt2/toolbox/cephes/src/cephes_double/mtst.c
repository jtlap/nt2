...
x =   cephes_cbrt(   cube(x) ):  max = 0.00E+00   rms = 0.00E+00
x =   cephes_atan(    cephes_tan(x) ):  max = 2.21E-16   rms = 3.27E-17
x =    cephes_sin(   cephes_asin(x) ):  max = 2.13E-16   rms = 2.95E-17
x =   cephes_sqrt( square(x) ):  max = 0.00E+00   rms = 0.00E+00
x =    cephes_log(    cephes_exp(x) ):  max = 1.11E-16 A rms = 4.35E-18 A
x =   cephes_tanh(  cephes_atanh(x) ):  max = 2.22E-16   rms = 2.43E-17
x =  cephes_asinh(   cephes_sinh(x) ):  max = 2.05E-16   rms = 3.49E-18
x =  cephes_acosh(   cephes_cosh(x) ):  max = 1.43E-15 A rms = 1.54E-17 A
x =  cephes_log10(  cephes_exp10(x) ):  max = 5.55E-17 A rms = 1.27E-18 A
x = cephes_pow( cephes_pow(x,a),1/a ):  max = 7.60E-14   rms = 1.05E-15
x =    cephes_cos(   cephes_acos(x) ):  max = 2.22E-16 A rms = 6.90E-17 A
*/
...
#else
double fabs(), cephes_sqrt(), cephes_cbrt(), cephes_exp(), cephes_log();
double cephes_exp10(), cephes_log10(), cephes_tan(), cephes_atan();
double cephes_sin(), cephes_asin(), cephes_cos(), cephes_acos(), cephes_pow();
double cephes_tanh(), cephes_atanh(), cephes_sinh(), cephes_asinh(), cephes_cosh(), cephes_acosh();
double cephes_gamma(), cephes_lgam(), cephes_jn(), cephes_yn(), ndtrl(), ndtril();
...
#if SETPREC
cephes_dprec();  /* set coprocessor precision */
#endif
ntr = NTRIALS;
...
#if GLIBC2
		y = lgamma(x);
		x = cephes_log( tgamma(x) );
#else
		y = cephes_lgam(x);
		x = cephes_log( cephes_gamma(x) );
#endif
		break;
...
rms = 1.0e-16 * cephes_sqrt( rmsa/m );
if(d->ctrl & RELERR)
