/* fltest.c
 * Test program for cephes_floor(), cephes_frexp(), cephes_ldexp()
 */
...
double cephes_frexp(), cephes_ldexp(), cephes_floor();
...
printf( "Testing cephes_frexp() and cephes_ldexp().\n" );
errfr = 0;
errld = 0;
underexp = 0;
f = 1.0;
...
for( i=0; i<2200; i++ )
	...
	if( (x != 0.0) && ((err > errth) || (e != e0)) )
		{
		printf( "Test %d: ", j+1 );
		printf( " cephes_frexp( %.15e) =?= %.15e * 2**%d;", x, y, e );
		printf( " should be %.15e * 2**%d\n", y0, e0 );
		errfr += 1;
		}
	y = cephes_ldexp( x, 1-e0 );
	err = y - 1.0;
	if( err < 0.0 )
		err = -err;
 ...
 * y = cephes_floor() forces z to be stored as a double and reloaded
...
if( errfl == 0 )
	printf( "No errors found in cephes_floor().\n" );
