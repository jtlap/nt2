...
extern double fabs(), cephes_floor(), cephes_log(), cephes_pow(), cephes_sqrt();

#ifdef Single
#define FLOAT float
#define FABS(x) (float)fabs((double)(x))
#define FLOOR(x) (float)cephes_floor((double)(x))
#define LOG(x) (float)cephes_log((double)(x))
#define POW(x,y) (float)cephes_pow((double)(x),(double)(y))
#define SQRT(x) (float)cephes_sqrt((double)(x))
#else
#define FLOAT double
#define FABS(x) fabs(x)
#define FLOOR(x) cephes_floor(x)
#define LOG(x) cephes_log(x)
#define POW(x,y) cephes_pow(x,y)
#define SQRT(x) cephes_sqrt(x)
#endif
...
(noted above) count as one flaw in the final tally below");
	...
	SqXMinX (Serious);
	if (J != Zero) Pause();
	printf("Testing if cephes_sqrt(X * X) == X for %d Integers X.\n", NoTrials);
	J = Zero;
	X = Two;
	Y = Radix;
	if ((Radix != One)) do  {
...
*/
	...
	/*=============================================*/
	printf("\n");
	/* ...calculate Exp2 == cephes_exp(2) == 7.389056099... */
	X = Zero;
	I = 2;
	Y = Two * Three;
	Q = Zero;
	...
	X = F9;
	Y = X - U1;
	printf("Testing X^((X + 1) / (X - 1)) vs. cephes_exp(2) = %.17e as X -> 1.\n",
		Exp2);
	for(I = 1;;) {
		Z = X - BInvrse;
		...
		if (Z != Zero) {
			...
			if (Y / (One - Radix * E9) < Z
			   || Y > (One + Radix + E9) * Z) {
				...
				printf(" is too far from cephes_sqrt(Z) ^ 2 = %.17e .\n", Y);
				}
			}
		}
	/*=============================================*/
