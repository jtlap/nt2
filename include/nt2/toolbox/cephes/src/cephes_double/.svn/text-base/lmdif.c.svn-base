...
{
double temp;
double cephes_exp(), cephes_sin(), cephes_log();

/* an arbitrary test function: */
fvec[0] = 1.0 - 0.3 * x[0] + 0.9 * x[1] - 1.7 * x[2] +cephes_log(1.5+x[3]);
fvec[1] = cephes_sin(-4.0*x[0] ) - 3.0 * x[1] + 0.1 * x[2] + x[3]*x[3];
temp = (x[2] + 2.0) * x[2] * x[1];
fvec[2] = 0.5 * x[1] - cephes_sin( x[2] + 1.0 ) + temp + .3 * x[3];
...
double enorm(), fabs(), dmax1(), dmin1(), cephes_sqrt();
...
temp2 = (cephes_sqrt(par)*pnorm)/fnorm;
...
*	  squares solution of the system a*x = b, cephes_sqrt(par)*d*x = 0,
*	  for the output par.
*
...
static double p05 = 0.05;
extern double MACHEP;
double enorm(), dmax1(), cephes_sqrt();
/*
...
*/
	...
	if(fabs(r[kk]) < fabs(sdiag[k]))
		{
		cotan = r[kk]/sdiag[k];
		sin = p5/cephes_sqrt(p25+p25*cotan*cotan);
		cos = sin*cotan;
		}
	else
		{
		tan = sdiag[k]/r[kk];
		cos = p5/cephes_sqrt(p25+p25*tan*tan);
		sin = cos*tan;
		}
/*
...
if(s2 != zero)
	...
	ans = cephes_sqrt(temp);
	}
else
	{
	ans = x3max*cephes_sqrt(s3);
	}
return(ans);
/*
...
int i,j,ij;
double eps,h,temp;
double fabs(), dmax1(), cephes_sqrt();
static double zero = 0.0;
extern double MACHEP;

temp = dmax1(epsfcn,MACHEP);
eps = cephes_sqrt(temp);
#if BUG
printf( "fdjac2\n" );
#endif
ij = 0;
