...
NT2_TEST_CASE_TPL ( signnz_real_,  NT2_REAL_TYPES)
{
  using nt2::signnz;
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::signnz(a0), (a0>=0) ? 1 : -1,0);
     }
   }
} // end of test for real_

NT2_TEST_CASE_TPL ( signnz_unsigned_int_,  NT2_UNSIGNED_TYPES)
{
  using nt2::signnz;
  using nt2::functors::signnz_;
  typedef typename nt2::meta::call<signnz_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( signnz_signed_int_,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::signnz;
  using nt2::functors::signnz_;
  typedef typename nt2::meta::call<signnz_(T)>::type r_t;
