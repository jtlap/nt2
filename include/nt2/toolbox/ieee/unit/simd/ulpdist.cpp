...
NT2_TEST_CASE_TPL(ulpdist, NT2_SIMD_TYPES )
{
 ...
 typedef typename nt2::meta::call<ulpdist_(n_t, n_t)>::type call_type;
 ...
 for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = nt2::random(-50.0, 50.0); // good value here for ulpdist
 }
   ...
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], ulpdist(a0[j], a1[j]) );
     }
 }
