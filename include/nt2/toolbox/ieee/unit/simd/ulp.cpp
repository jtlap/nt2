...
NT2_TEST_CASE_TPL(ulp, NT2_SIMD_SIGNED_TYPES )
{
 ...
 typedef typename nt2::meta::call<ulp_(n_t)>::type call_type;

 NT2_TEST( (boost::is_same<call_type, n_t>::value) );  
 NT2_ALIGNED_TYPE(T) data[1*cardinal_of<n_t>::value]; 
 for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
   data[i] = nt2::random(-100.0, 100.0); // good value here for ulp
 }
   n_t a0 = load<n_t>(&data[0],0); 
   n_t v  = ulp(a0);
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
   {
     NT2_TEST_EQUAL( v[j], ulp(a0[j]) );
   }
 }
NT2_TEST_CASE_TPL(unulped_ulp, NT2_SIMD_UNSIGNED_TYPES )
{
 ...
 NT2_ALIGNED_TYPE(T) data[1*cardinal_of<n_t>::value]; 
 for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
   data[i] = nt2::random(0.0, 100.0); // good value here for ulp
 }
   n_t a0 = load<n_t>(&data[0],0); 
   n_t v  = ulp(a0);
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
