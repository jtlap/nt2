                         ...
                         )
{
 ...
 typedef typename nt2::meta::call<exponentbits_(n_t)>::type call_type;

 NT2_TEST( (boost::is_same<call_type, in_t>::value) );
 NT2_ALIGNED_TYPE(T) data[1*cardinal_of<n_t>::value];
 for(int i=0;i<1*cardinal_of<n_t>::value;++i)
   data[i] = 5*(i-T(cardinal_of<n_t>::value)/2); // good value here for exponentbits

 n_t a0 = load<n_t>(&data[0],0);  
 in_t v  = nt2::exponentbits(a0);
 for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
   { 
     NT2_TEST_EQUAL( v[j], exponentbits(a0[j]) );
   } 
}
