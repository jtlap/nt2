...
NT2_TEST_CASE_TPL(signnz, NT2_SIMD_SIGNED_TYPES )
{
 ...
 typedef typename nt2::meta::call<signnz_(n_t)>::type call_type;

 NT2_TEST( (boost::is_same<call_type, n_t>::value) );  
 NT2_ALIGNED_TYPE(T) data[1*cardinal_of<n_t>::value]; 
 for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
   data[i] = nt2::random(-100.0, 100.0); // good value here for signnz
 }
   n_t a0 = load<n_t>(&data[0],0); 
   n_t v  = signnz(a0);
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
   {
     NT2_TEST_EQUAL( v[j], signnz(a0[j]) );
   }
 }
NT2_TEST_CASE_TPL(unsigned_signnz, NT2_SIMD_UNSIGNED_TYPES )
{
 ...
 NT2_ALIGNED_TYPE(T) data[1*cardinal_of<n_t>::value]; 
 for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
   data[i] = nt2::random(0.0, 100.0); // good value here for signnz
 }
   n_t a0 = load<n_t>(&data[0],0); 
   n_t v  = signnz(a0);
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
