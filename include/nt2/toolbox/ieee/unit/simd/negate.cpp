...
NT2_TEST_CASE_TPL(negate, NT2_SIMD_SIGNED_TYPES )
{
 using nt2::negate;
 ...
 typedef typename nt2::meta::call<negate_(n_t, n_t)>::type call_type;
 ...
 for(int i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = nt2::random(-10.0, 10.0); // good value here for negate
 }
   ...
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], negate(a0[j], a1[j]) );
     }
 }
