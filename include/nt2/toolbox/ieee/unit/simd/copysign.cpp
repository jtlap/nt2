...
NT2_TEST_CASE_TPL(copysign, NT2_SIMD_TYPES )
{
 ...
 typedef typename nt2::meta::call<copysign_(n_t, n_t)>::type call_type;
 ...
 for(int i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = i-T(cardinal_of<n_t>::value)/2; // good value here for copysign
 }
   n_t a0 = load<n_t>(&data[0],0);   
   n_t a1 = load<n_t>(&data[0],1);
   n_t v  = copysign(a0, a1);
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], copysign(a0[j], a1[j]) );
     }
 }
