                         ...
                         )
{
 ...
 typedef typename nt2::meta::call<fast_frexp_(n_t)>::type call_type;

  
 NT2_TEST( (boost::is_same<call_type, rn_t>::value) );
 NT2_ALIGNED_TYPE(T) data[1*cardinal_of<n_t>::value];
 for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i)
   data[i] = nt2::random(-10.0, 10.0); // good value here for fast_frexp

 n_t a0 = load<n_t>(&data[0],0);  
 rn_t r = nt2::fast_frexp(a0);
 for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
   {
     sn_t m =  fast_frexp(a0[j]); 
