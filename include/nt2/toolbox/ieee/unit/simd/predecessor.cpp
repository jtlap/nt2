                         ...
                         )
{
 ...
 typedef typename nt2::meta::call<predecessor_(n_t)>::type call_type;

 NT2_TEST( (boost::is_same<call_type, n_t>::value) );
 NT2_ALIGNED_TYPE(T) data[1*cardinal_of<n_t>::value];
 for(int i=0;i<1*cardinal_of<n_t>::value;++i)
   data[i] =nt2::random(-100.0, 100.0); // good value here for predecessor

 n_t a0 = load<n_t>(&data[0],0);  
 n_t v  = nt2::predecessor(a0);
