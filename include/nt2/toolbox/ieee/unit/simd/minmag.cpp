...
NT2_TEST_CASE_TPL(minmag, NT2_SIMD_SIGNED_TYPES )
{
 ...
 typedef typename nt2::meta::call<minmag_(n_t, n_t)>::type call_type;
 ...
 for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = nt2::random(-100.0, 100.0); // good value here for minmag
 }
   ...
   for(std::size_t j=0;j<cardinal_of<n_t>::value;++j) 
     {
       NT2_TEST_EQUAL( v[j], minmag(a0[j], a1[j]) );
     }
 }
...
NT2_TEST_CASE_TPL(unsigned_minmag, NT2_SIMD_UNSIGNED_TYPES )
{
 ...
 typedef typename nt2::meta::call<minmag_(n_t, n_t)>::type call_type;
  ...
  NT2_ALIGNED_TYPE(T) data[2*cardinal_of<n_t>::value]; 
 for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){    
   data[i] = nt2::random(0.0, 100.0); // good value here for minmag
 }
   n_t a0 = load<n_t>(&data[0],0);   
   n_t a1 = load<n_t>(&data[0],1);
   n_t v  = minmag(a0, a1);
