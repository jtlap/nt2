...
NT2_TEST_CASE_TPL ( expint_real__1,  NT2_REAL_TYPES)
{
    ...
    for (uint32_t j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::boost_math::expint(a0),nt2::boost_math::expint(a0),1);
...
NT2_TEST_CASE_TPL ( expint_real__2,  NT2_REAL_TYPES)
{
    ...
    for (uint32_t j =0; j < NR; ++j )
      {
        std::cout << "for params "
                  ...
                  << ", a1 = "<< u_t(a1 = tab_a1[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::boost_math::expint(a0,a1),nt2::boost_math::expint(a0,a1),1);
        ulp0=nt2::max(ulpd,ulp0);
     }
     std::cout << "max ulp found is: " << ulp0 << std::endl;
   }
