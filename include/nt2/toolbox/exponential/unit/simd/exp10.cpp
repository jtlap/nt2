...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(exp10, (double)(float) )
{
 using nt2::exp10;
 using nt2::functors::exp10_;    
 using nt2::load; 
 ...
 typedef typename nt2::meta::call<exp10_(n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(0.0, 100.0); // good value here for exp10
     }
     n_t a0 = load<n_t>(&data[0],0); 
     rn_t v  = exp10(a0);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
	 NT2_TEST_LESSER( z = nt2::ulpdist(v[j], exp10(a0[j])), 1);
	 if (z > m) m = z; 
       }
   }
