...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(expm1, (double)(float))//NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 using nt2::expm1;
 ...
 typedef typename nt2::meta::call<expm1_(n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(-1.0, 100.0); // good value here for expm1
     }
     n_t a0 = load<n_t>(&data[0],0); 
     rn_t v  = expm1(a0);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
       NT2_TEST_LESSER( z = nt2::ulpdist(v[j], expm1(a0[j])), 1);
       if (z > m) m = z; 
       }
   }
