...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(pow, (double)(float))//NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 using nt2::pow;
 using nt2::tag::pow_;    
 using nt2::load; 
 ...
 typedef typename nt2::meta::call<pow_(n_t, n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(0.0, 5.0); // good value here for pow
     }
     ...
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
       NT2_TEST_LESSER( z = nt2::ulpdist(v[j], pow(a0[j], a1[j])), 10);
       if (z > m) m = z; 
       }
   }
