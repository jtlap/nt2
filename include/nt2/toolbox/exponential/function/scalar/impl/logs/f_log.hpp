...
namespace nt2
{
  namespace details
  {
				      ...
				      A0& y)
	{
	  ...
	  x += b_and(x, x_lt_sqrthf)+single_constant<A0,0xbf800000>();
	  x2 = sqr(x);
	...
	static inline A0 log(const  A0& a0)
	{
	  ...
	  A0 z  = x + y;
	  return madd(single_constant<A0, 0x3f318000>(), fe, z);
	}
	...
	static inline A0 log2(const  A0& a0)
	{
	  ...
	  A0 z = madd( x
		       ...
		       , mul(y,single_constant<A0, 0x3ee2a8ed>())// 0.44269504088896340735992
		       );
	  return ((z+y)+x)+fe;
	}
	...
	static inline A0 log10(const  A0& a0)
	{
	  ...
	  return amul(z, fe, single_constant<A0, 0x3e9a0000 >());//2.48745663981195213739E-4f // log10(2)lo
	}
      }; 
    }
  }
