 ...
 ******************************************************************************/
#ifndef NT2_CORE_NUMERIC_FUNCTION_DETAILS_SCALAR_IMPL_TRIGO_D_INVTRIG_HPP_INCLUDED
#define NT2_CORE_NUMERIC_FUNCTION_DETAILS_SCALAR_IMPL_TRIGO_D_INVTRIG_HPP_INCLUDED
    ...
    namespace internal
    {
      inline double asin(const  double& a0)
      {
        ...
        if ((x <  Sqrteps<A0>())) return a0;
              A0 zz;
        if((x >  Const<double,0x3fe4000000000000ll> ())) //0.625;
        {
          zz = oneminus(x);
          ...
          zz =  sqrt(zz+zz);
          A0 z = Pio_4<A0>()-zz;
          zz = fma(zz, vp, Const<double,0xbc91a62633145c07ll>());
          z =  z-zz;
          zz = z+Pio_4<A0>();
        }
        else
      ...
      double acos(const  double& a0)
      {
        ...
        z = z + Const<double,0x3c91a62633145c07ll>();//6.123233995736765886130E-17
        z = z + Pio_4<A0>();
        return( z );
      }
      ...
      {
        typedef double A0;
        //	static const A0 tanpio8 = Const<double, 0x3fda827999fcef31ll>();
        if (iseqz(a0))  return a0;
        if (isinf(a0)) return Pio_2<A0>()*sign(a0);
        A0 x =  abs(a0);
        A0 y;
        A0 flag = (x >  Const<double,0x4003504f333f9de6ll>());
        if (flag)
          {
            y =  Pio_2<A0>();
            x =  -rec(x);
          }
        else if ((x <=  Const<double,0x3fe51eb851eb851fll>()))
          {
            y = Zero<A0>();
          }
        else
        ...
        z = z*horner< NT2_HORNER_COEFF(double, 5,
              ...
              z = fma(x, z, x);
        static const A0 morebits = Const<double,0x3c91a62633145c07ll>();
        z += flag * morebits;
        y = y + z;
        if( isltz(a0) )  y = -y;
        return(y);
