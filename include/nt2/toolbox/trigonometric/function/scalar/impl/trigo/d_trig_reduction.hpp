  ...
  namespace details
  {
    namespace internal
    {
      ...
      template < class A0, class precision_tag> struct trig_reduction < A0, radian_tag, precision_tag, double>
      {
        ...
        static inline logic isalreadyreduced(const A0&a0){ return is_lt(a0,Pio_4<A0>()); }
        static inline logic ismedium (const A0&a0){return is_le(a0,integral_constant<A0,0x412921fb54442d18ll>()); }
        static inline logic issmall  (const A0&a0){return is_le(a0,integral_constant<A0,0x404f6a7a2955385ell>()); }
        ...
        static inline void sincos_replacement(const A0& a0, A0&s, A0&c){ ::sincos(a0, &s, &c); }
	...
	static inline logic replacement_needed(const A0& a0) {return False<A0>();details::ignore_unused(a0); }//return a0 > integral_constant<A0, Med>();}//
	static inline logic replacement_available(){return True<A0>(); }
	static inline logic isalreadyreduced(const A0&a0){return is_lt(a0,Pio_4<A0>()); }
	static inline logic issmall  (const A0&a0){return is_le(a0,integral_constant<A0,0x404f6a7a2955385ell>()); }
	static inline A0 cos_replacement(const A0& a0){ return std::cos(a0); }
	...
	static inline logic replacement_needed(const A0& a0) {return is_gt(a0,integral_constant<A0,0x416fffffe0000000ll>());}
	static inline logic replacement_available(){return True<A0>(); }
	...
	static inline void sincos_replacement(const A0& a0, A0&s, A0&c){ ::sincos(inrad(a0), &s, &c); }
        static inline logic cot_invalid(const A0& x) { return isinvalid(x)|iseven(x/Ninety<A0>()); }
        static inline logic tan_invalid(const A0& x) { return isinvalid(x)|isodd(x/Ninety<A0>()); }
	static inline int_type reduce(A0 x, A0& xr, A0& xc)
	{
	  A0 xi = round2even(x*integral_constant<A0,0x3f86c16c16c16c17ll>());//1.111111111111111e-02
	  x -= xi * Ninety<A0>();//90.0f
	  xr =  x*integral_constant<A0,0x3f91df46a2529d39ll>();//0.0174532925199432957692
	  xc = Zero<A0>();
	  return toint(xi);
	}
      };
	...
	// cotpi*x) is thus Nan
	static inline logic replacement_needed(const A0& ) {return False<A0>(); }//return a0 > integral_constant<A0,bm::integral_c<uint64_t, Big> >();}
	static inline logic replacement_available(){return True<A0>(); }
