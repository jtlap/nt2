...
namespace nt2
{
  namespace details
  {
	...
	static inline A0 asin(const  A0& a0)
	{
	  ...
	  x = abs(a0);
	  sign = bitofsign(a0);
	  A0 x_smaller_1e_4 = islt(x, integral_constant<A0, 0x38d1b717>()); //1.0e-4f;
	...
	static inline A0 atan(const  A0& a0)
	{
	  ...
	  sign = bitofsign(a0);
	  //	bf::tie(sign, x) = sign_and_abs(a0);
	  const A0 flag1 = islt(x, integral_constant<A0, 0x401a827a>()); //tan3pio8);
	  const A0 flag2 = b_and(isge(x, integral_constant<A0, 0x3ed413cd>()), flag1);
