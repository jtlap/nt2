...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(atand, NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 using nt2::atand;
 using nt2::functors::atand_;    
 using nt2::load; 
 ...
 typedef typename nt2::meta::call<atand_(n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(-1.5, 1.5); // good value here for atand
     }
     n_t a0 = load<n_t>(&data[0],0); 
     rn_t v  = atand(a0);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
	 NT2_TEST_LESSER( z = nt2::ulpdist(v[j], atand(a0[j])), 1);
	 if (z > m) m = z; 
       }
   }
