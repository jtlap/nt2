...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(secd, NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 using nt2::secd;
 using nt2::tag::secd_;    
 using nt2::load; 
 ...
 typedef typename nt2::meta::call<secd_(n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(0., 45.); // good value here for secd
     }
     n_t a0 = load<n_t>(&data[0],0); 
     rn_t v  = secd(a0);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
       NT2_TEST_LESSER_EQUAL( z = nt2::ulpdist(v[j], secd(a0[j])), 1);
       std::cout  << "  " << a0[j] << v[j] << "  " << secd(a0[j]) << std::endl;
       if (z > m) m = z; 
       }
   }
