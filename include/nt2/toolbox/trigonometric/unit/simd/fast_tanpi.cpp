...
NT2_TEST_CASE_TPL ( fast_tanpi_real_convert__1,  NT2_REAL_CONVERTIBLE_TYPES)
{
  ...
  {
    NT2_CREATE_SIMD_BUFFER(a0,T, NR, T(0.25), T(0.25));
    ...
    for(int j = 0; j < NR/cardinal_of<n_t>::value; j++)
      {
        ...
        for(int i = 0; i< cardinal_of<n_t>::value; i++)
        {
          ...
          NT2_TEST_ULP_EQUAL( v[i],nt2::fast_tanpi(tab_a0[k]),1.5);
