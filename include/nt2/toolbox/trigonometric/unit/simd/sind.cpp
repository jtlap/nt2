...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(sind, NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 using nt2::sind;
 using nt2::tag::sind_;    
 using nt2::load; 
 ...
 typedef typename nt2::meta::call<sind_(n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(0.0, 100.0); // good value here for sind
     }
     n_t a0 = load<n_t>(&data[0],0); 
     rn_t v  = sind(a0);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
       NT2_TEST_LESSER_EQUAL( z = nt2::ulpdist(v[j], sind(a0[j])), 1);
       if (z > m) m = z; 
       }
   }
