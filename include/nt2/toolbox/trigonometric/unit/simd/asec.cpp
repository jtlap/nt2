...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(asec, NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 using nt2::asec;
 using nt2::functors::asec_;    
 using nt2::load; 
 ...
 typedef typename nt2::meta::call<asec_(n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(1.0, 10.0); // good value here for asec
     }
     n_t a0 = load<n_t>(&data[0],0); 
     rn_t v  = asec(a0);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
	 NT2_TEST_LESSER( z = nt2::ulpdist(v[j], asec(a0[j])), 1);
	 if (z > m) m = z; 
       }
   }
