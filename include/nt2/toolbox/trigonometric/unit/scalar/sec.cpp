...
NT2_TEST_CASE_TPL ( sec_real__1,  NT2_REAL_TYPES)
{
  using nt2::sec;
  using nt2::tag::sec_;
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::sec(a0),nt2::rec(nt2::crlibm::cos<nt2::rn>(a0)),1.0);
...
NT2_TEST_CASE_TPL ( sec_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::sec;
  using nt2::tag::sec_;
  typedef typename nt2::meta::call<sec_(T)>::type r_t;
