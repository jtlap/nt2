    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::acsc(a0),nt2::crlibm::asin<nt2::rn>(nt2::rec(a0)),1.0);
...
NT2_TEST_CASE_TPL ( acsc_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::acsc;
  using nt2::tag::acsc_;
  typedef typename nt2::meta::call<acsc_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( acsc_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::acsc;
  using nt2::tag::acsc_;
  typedef typename nt2::meta::call<acsc_(T)>::type r_t;
