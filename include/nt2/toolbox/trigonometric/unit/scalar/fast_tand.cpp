...
NT2_TEST_CASE_TPL ( fast_tand_real__1,  NT2_REAL_TYPES)
{
  using nt2::fast_tand;
  ...
  {
    NT2_CREATE_BUFFER(a0,T, 100, T(-45), T(45));
    double ulp0 = 0.0;
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::fast_tand(a0),nt2::crlibm::tan<nt2::rn>(a0*nt2::Deginrad<T>()),2.0);
...
NT2_TEST_CASE_TPL ( fast_tand_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::fast_tand;
  using nt2::tag::fast_tand_;
  typedef typename nt2::meta::call<fast_tand_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( fast_tand_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::fast_tand;
  using nt2::tag::fast_tand_;
  typedef typename nt2::meta::call<fast_tand_(T)>::type r_t;
