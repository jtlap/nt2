...
NT2_TEST_CASE_TPL ( cotpi_real__1,  NT2_REAL_TYPES)
{
  using nt2::cotpi;
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::cotpi(a0),nt2::rec(nt2::crlibm::tanpi<nt2::rn>(a0)),1.0);
...
NT2_TEST_CASE_TPL ( cotpi_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::cotpi;
  using nt2::tag::cotpi_;
  typedef typename nt2::meta::call<cotpi_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( cotpi_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::cotpi;
  using nt2::tag::cotpi_;
  typedef typename nt2::meta::call<cotpi_(T)>::type r_t;
