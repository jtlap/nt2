...
NT2_TEST_CASE_TPL ( tand_real__1,  NT2_REAL_TYPES)
{
  ...
  {
    NT2_CREATE_SCALAR_BUFFER(a0,T, NR, T(-179), T(179));
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::tand(a0),::tanl(a0*nt2::long_deginrad),1.0);
...
NT2_TEST_CASE_TPL ( tand_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::tand;
  using nt2::tag::tand_;
  typedef typename nt2::meta::as_integer<T>::type iT;
...
NT2_TEST_CASE_TPL ( tand_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::tand;
  using nt2::tag::tand_;
  typedef typename nt2::meta::as_integer<T>::type iT;
