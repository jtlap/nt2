...
NT2_TEST_CASE_TPL ( fast_tanpi_real__1,  NT2_REAL_TYPES)
{
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::fast_tanpi(a0),::tan(nt2::Pi<T>()*a0),1.0);
...
NT2_TEST_CASE_TPL ( fast_tanpi_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::fast_tanpi;
  using nt2::tag::fast_tanpi_;
  typedef typename nt2::meta::as_integer<T>::type iT;
...
NT2_TEST_CASE_TPL ( fast_tanpi_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::fast_tanpi;
  using nt2::tag::fast_tanpi_;
  typedef typename nt2::meta::as_integer<T>::type iT;
