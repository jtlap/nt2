...
NT2_TEST_CASE_TPL ( cos_real__1,  NT2_REAL_TYPES)
{
  using nt2::cos;
  using nt2::tag::cos_;
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::cos(a0),nt2::crlibm::cos<nt2::rn>(a0),0.5);
...
NT2_TEST_CASE_TPL ( cos_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::cos;
  using nt2::tag::cos_;
  typedef typename nt2::meta::call<cos_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( cos_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::cos;
  using nt2::tag::cos_;
  typedef typename nt2::meta::call<cos_(T)>::type r_t;
