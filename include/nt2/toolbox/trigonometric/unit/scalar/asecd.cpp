...
NT2_TEST_CASE_TPL ( asecd_real__1,  NT2_REAL_TYPES)
{
  using nt2::asecd;
  ...
  {
    NT2_CREATE_BUFFER(a0,T, 100, nt2::One<T>(), nt2::Ten<T>());
    double ulp0 = 0.0;
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::asecd(a0),nt2::crlibm::acos<nt2::rn>(nt2::rec(a0))*nt2::Radindeg<T>(),2.0);
...
NT2_TEST_CASE_TPL ( asecd_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::asecd;
  using nt2::tag::asecd_;
  typedef typename nt2::meta::call<asecd_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( asecd_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::asecd;
  using nt2::tag::asecd_;
  typedef typename nt2::meta::call<asecd_(T)>::type r_t;
