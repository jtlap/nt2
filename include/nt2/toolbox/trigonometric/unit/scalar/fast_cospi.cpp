    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::fast_cospi(a0),nt2::crlibm::cospi<nt2::rn>(a0),0.5);
...
NT2_TEST_CASE_TPL ( fast_cospi_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::fast_cospi;
  using nt2::tag::fast_cospi_;
  typedef typename nt2::meta::call<fast_cospi_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( fast_cospi_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::fast_cospi;
  using nt2::tag::fast_cospi_;
  typedef typename nt2::meta::call<fast_cospi_(T)>::type r_t;
