...
namespace nt2 { namespace functors
{
  ...
  struct call<ellpk_,tag::scalar_(tag::arithmetic_),Info>
  {
      ...
      {
        if (a0>One<A0>()||(isltz(a0)))
          return Nan<A0>();
        else if (a0<=Eps<A0>())
          return integral_constant<A0,0x3fb17218>() - Half<A0>()*nt2::log(a0);
        else
      ...
      NT2_FUNCTOR_CALL_EVAL_IF(1, double)
      {
        if (a0>One<A0>()||(isltz(a0)))
          ...
          return integral_constant<A0,0x3ff62e42fefa39efll>() - Half<A0>()*log(a0);
        else
