...
namespace nt2 { namespace ext
{
  ...
  struct call<tag::is_gtz_(tag::simd_(tag::arithmetic_, tag::see_)),
...
{
  template<class Dummy>
  struct call<tag::is_gtz_(tag::simd_(tag::unsigned_, tag::see_)),
              tag::cpu_, Dummy> : callable
  {
    template<class Sig> struct result;
    template<class This,class A0> struct result<This(A0)> : meta::strip<A0>{};//
...
{
  template<class Dummy>
  struct call<tag::is_gtz_(tag::simd_(tag::int64_, tag::see_)),
              tag::cpu_, Dummy> : callable
  {
    template<class Sig> struct result;
    template<class This,class A0> struct result<This(A0)> : meta::strip<A0>{};//
