...
NT2_TEST_CASE_TPL ( is_negative_real__1,  NT2_REAL_TYPES)
{
  ...
  NT2_TEST_ULP_EQUAL(  is_negative(-nt2::Zero<T>()), nt2::True<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  is_negative(nt2::Half<T>()), nt2::False<r_t>(), 0.5);
  ...
  NT2_TEST_ULP_EQUAL(  is_negative(nt2::Zero<T>()), nt2::False<r_t>(), 0.5);
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::is_negative(a0),nt2::bitofsign(a0)!=0,0);
...
NT2_TEST_CASE_TPL ( is_negative_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  ...
  // specific values tests
  NT2_TEST_ULP_EQUAL(  is_negative(nt2::Mone<T>()), nt2::False<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  is_negative(nt2::One<T>()), nt2::False<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  is_negative(nt2::Two<T>()), nt2::False<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  is_negative(nt2::Zero<T>()), nt2::False<r_t>(), 0.5);
  // random verifications
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::is_negative(a0),a0<=0,0);
        ulp0=nt2::max(ulpd,ulp0);
     }
     std::cout << "max ulp found is: " << ulp0 << std::endl;
   }
