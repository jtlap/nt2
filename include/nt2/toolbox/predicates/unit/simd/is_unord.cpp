...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(is_unord, NT2_SIMD_TYPES )
{
 using nt2::is_unord;
 ...
 typedef typename nt2::meta::call<is_unord_(n_t, n_t)>::type call_type;
 ...
 for(int j =  0;  j < 10; j++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(-10000.0, 10000.0); // good value here for is_unord
     }
     n_t a0 = load<n_t>(&data[0],0); 
     n_t a1 = load<n_t>(&data[0],1); 
     n_t v  = is_unord(a0, a1);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       { 
	 NT2_TEST_EQUAL(nt2::boolean(v[j]), is_unord(a0[j],a1[j] ));
       }
   }
}
