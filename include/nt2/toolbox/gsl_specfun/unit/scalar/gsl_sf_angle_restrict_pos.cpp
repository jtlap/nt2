...
NT2_TEST_CASE_TPL ( gsl_sf_angle_restrict_pos_real__1,  NT2_REAL_TYPES)
{
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::gsl_specfun::gsl_sf_angle_restrict_pos(a0),nt2::gsl_specfun::gsl_sf_angle_restrict_pos(a0),1);
