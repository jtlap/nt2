...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(gcd,NT2_TYPES )
{
 using nt2::gcd;
 using nt2::tag::gcd_;    
 using nt2::load;  
 ...
 typedef typename nt2::meta::call<gcd_(n_t, n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(1, 100); // good value here for gcd
     }
     ...
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
	 NT2_TEST_EQUAL( v[j], gcd(a0[j], a1[j]));
	 std::cout << int(a0[j]) << "  " <<  int(a1[j])<< "  " << int(v[j])<< "  " << int(gcd(a0[j], a1[j])) << std::endl; 
       }
   } 
} 
