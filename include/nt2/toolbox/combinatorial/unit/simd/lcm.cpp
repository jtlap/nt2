...
NT2_TEST_CASE_TPL(lcm,NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 ...
 typedef typename nt2::meta::call<lcm_(n_t, n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(1, 100); // good value here for lcm
     }
     ...
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
	 NT2_TEST_LESSER_EQUAL( z = nt2::ulpdist(v[j], lcm(a0[j], a1[j])), 1);
	 std::cout << a0[j] << "  " <<  a1[j]<< "  " << v[j]<< "  " << lcm(a0[j], a1[j]) << std::endl; 
	 if (z > m) m = z; 
       }
   }
