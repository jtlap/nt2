...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(cnp,NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 using nt2::cnp;
 using nt2::tag::cnp_;    
 using nt2::load; 
 ...
 typedef typename nt2::meta::call<cnp_(n_t, n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<2*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(1, 5); // good value here for cnp
     }
     ...
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
	 NT2_TEST_LESSER_EQUAL( z = nt2::ulpdist(v[j], cnp(a0[j], a1[j])), 1);
	 std::cout << a0[j] << "  " <<  a1[j]<< "  " << v[j]<< "  " << cnp(a0[j], a1[j]) << std::endl; 
	 if (z > m) m = z; 
       }
   }
