...
{
  using nt2::fdlibm::log1p; 
  using nt2::fdlibm::tag::log1p_;
  const int N = 2; 
   NT2_TEST( (boost::is_same < typename nt2::meta::call<log1p_(T)>::type
            , T
            >::value)
           );
   typedef typename boost::result_of<nt2::meta::floating(T)>::type r_t; 
   NT2_TEST_EQUAL(  nt2::fdlibm::log1p( T(0) )  , 0 );
   NT2_TEST_EQUAL(  nt2::fdlibm::log1p( T(-1) )  , nt2::Minf<T>() );
   NT2_TEST_EQUAL(  nt2::fdlibm::log1p( nt2::Inf<T>() )  , nt2::Inf<T>() );
   NT2_TEST_EQUAL(  nt2::fdlibm::log1p(nt2::Exp_1<T>()-1 ), T(1)); 
   NT2_TEST_LESSER_EQUAL(  nt2::ulpdist(nt2::fdlibm::log1p(nt2::Eps<T>()), nt2::Eps<T>()), 1); 
   ...
   for(int i=0; i < 10; i++)  
     {
       NT2_TEST_LESSER_EQUAL(nt2::ulpdist(nt2::fdlibm::log1p(v[i]), t[i]), 3);
       std::cout << std::setprecision(20) << v[i] << "  " << nt2::fdlibm::log1p(v[i]) << "  " << t[i] << std::endl; 
     } }
