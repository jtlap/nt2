...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(splatted_prod, NT2_SIMD_TYPES) 
{
 ...
 typedef typename nt2::meta::call<splatted_prod_(n_t)>::type call_type;
 ...
 for(int j =  0;  j < 10; j++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = cardinal_of<n_t>::value-i-1; // good value here for splatted_prod
     }
     n_t a0 = load<n_t>(&data[0],0); 
     n_t v  = splatted_prod(a0);
     std::cout << "  " << a0 << "  " << v << std::endl; 
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       { 
	 NT2_TEST_EQUAL(v[j], nt2::prod(a0));
       }
   }
} 
