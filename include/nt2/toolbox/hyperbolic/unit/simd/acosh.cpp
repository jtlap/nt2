...
//////////////////////////////////////////////////////////////////////////////
NT2_TEST_CASE_TPL(acosh, NT2_SIMD_REAL_CONVERTIBLE_TYPES )
{
 using nt2::acosh;
 using nt2::functors::acosh_;    
 using nt2::load; 
 ...
 typedef typename nt2::meta::call<acosh_(n_t)>::type call_type;
 ...
 for(int num = 0; num < 10; num++)
   {
     for(std::size_t i=0;i<1*cardinal_of<n_t>::value;++i){
       data[i] = nt2::random(1.0, 100.0); // good value here for acosh
     }
     n_t a0 = load<n_t>(&data[0],0); 
     rn_t v  = acosh(a0);
     for(std::size_t j=0;j<cardinal_of<n_t>::value;++j)
       {
	 NT2_TEST_LESSER( z = nt2::ulpdist(v[j], acosh(a0[j])), 1);
	 if (z > m) m = z; 
       }
   }
