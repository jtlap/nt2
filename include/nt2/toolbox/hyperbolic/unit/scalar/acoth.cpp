...
//////////////////////////////////////////////////////////////////////////////
#define NT2_UNIT_MODULE "nt2 hyperbolic toolbox - acoth/scalar Mode"
...
NT2_TEST_CASE_TPL ( acoth_real__1,  NT2_REAL_TYPES)
{
  using nt2::acoth;
  ...
  NT2_TEST_ULP_EQUAL(  acoth(nt2::Inf<T>()), nt2::Zero<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  acoth(nt2::Minf<T>()), nt2::Zero<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  acoth(nt2::Mone<T>()), nt2::Minf<r_t>(), 0.5);
  NT2_TEST_ULP_EQUAL(  acoth(nt2::Nan<T>()), nt2::Nan<r_t>(), 0.5);
  ...
  {
    NT2_CREATE_BUFFER(a0,T, NR, T(1), T(1000));
    double ulp0 = 0.0;
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
  ...
  {
    NT2_CREATE_BUFFER(a0,T, NR, T(-1000), T(-1));
    double ulp0 = 0.0;
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
...
NT2_TEST_CASE_TPL ( acoth_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::acoth;
  using nt2::tag::acoth_;
  typedef typename nt2::meta::call<acoth_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( acoth_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::acoth;
  using nt2::tag::acoth_;
  typedef typename nt2::meta::call<acoth_(T)>::type r_t;
