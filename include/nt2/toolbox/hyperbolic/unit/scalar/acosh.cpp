...
NT2_TEST_CASE_TPL ( acosh_real__1,  NT2_REAL_TYPES)
{
  using nt2::acosh;
    ...
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
                  << "  a0 = "<< u_t(a0 = tab_a0[j])
                  << std::endl;
        NT2_TEST_ULP_EQUAL( nt2::acosh(a0),nt2::boost_math::acosh(a0),0.5);
...
NT2_TEST_CASE_TPL ( acosh_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::acosh;
  using nt2::tag::acosh_;
  typedef typename nt2::meta::call<acosh_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( acosh_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::acosh;
  using nt2::tag::acosh_;
  typedef typename nt2::meta::call<acosh_(T)>::type r_t;
