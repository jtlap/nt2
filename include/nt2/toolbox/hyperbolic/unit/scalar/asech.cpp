...
NT2_TEST_CASE_TPL ( asech_real__1,  NT2_REAL_TYPES)
{
  using nt2::asech;
  ...
  {
    NT2_CREATE_BUFFER(a0,T, 100, T(0), T(0.9));
    double ulp0 = 0.0;
    for (int j =0; j < NR; ++j )
      {
        std::cout << "for param "
...
NT2_TEST_CASE_TPL ( asech_unsigned_int__1,  NT2_UNSIGNED_TYPES)
{
  using nt2::asech;
  using nt2::tag::asech_;
  typedef typename nt2::meta::call<asech_(T)>::type r_t;
...
NT2_TEST_CASE_TPL ( asech_signed_int__1,  NT2_INTEGRAL_SIGNED_TYPES)
{
  using nt2::asech;
  using nt2::tag::asech_;
  typedef typename nt2::meta::call<asech_(T)>::type r_t;
